...
namespace CryptoNote
{
  class BlockWithTransactions : public IBlock
  {
   public:
    ...
    virtual size_t getTransactionCount() const override { return transactions.size(); }
  ...
  size_t core::get_alternative_blocks_count() { return m_blockchain.getAlternativeBlocksCount(); }
  //-----------------------------------------------------------------------------------------------
  bool core::init(const CoreConfig& config, const MinerConfig& minerConfig, bool load_existing)
  {
    ...
    if (!(r))
    {
      logger(ERROR, BRIGHT_RED) << "<< Core.cpp << "
  ...
  bool core::set_genesis_block(const Block& b) { return m_blockchain.resetAndSetGenesisBlock(b); }
  ...
  {  // Deprecated. Should be removed with CryptoNoteProtocolHandler.
    ...
    {
      logger(INFO) << "<< Core.cpp << "
                   << "WRONG TRANSACTION BLOB, too big size " << tx_blob.size() << "<< Core.cpp << "
                   << ", rejected";
  ...
  bool core::check_tx_semantic(const Transaction& tx, bool keeped_by_block, uint32_t& height)
  {
    if (!tx.inputs.size())
    {
      logger(ERROR) << "<< Core.cpp << "
                    << "tx with empty inputs, rejected for tx id= " << getObjectHash(tx);
      return false;
    }
    ...
    {
      logger(ERROR) << "<< Core.cpp << "
                    << "unsupported input types for tx id= " << getObjectHash(tx);
      return false;
    }
    ...
    {
      logger(ERROR) << "<< Core.cpp << "
                    << "tx with invalid outputs, rejected for tx id= " << getObjectHash(tx)
                    << "<< Core.cpp << "
                    << ": " << errmsg;
      return false;
    }
    ...
    {
      logger(ERROR) << "<< Core.cpp << "
                    << "tx have money overflow, rejected for tx id= " << getObjectHash(tx);
      return false;
    }
    ...
    if (amount_in < amount_out)
    {
      ...
      uint32_t testHeight =
          height > parameters::END_MULTIPLIER_BLOCK ? 0 : (uint32_t)(-1);  // try other mode
      amount_in = m_currency.getTransactionAllInputsAmount(tx, testHeight);
      if (amount_in < amount_out)
      {
        logger(ERROR) << "<< Core.cpp << "
                      << "tx with wrong amounts: ins " << amount_in << ", outs " << amount_out
    ...
    {
      logger(ERROR) << "<< Core.cpp << "
                    << "tx has a few inputs with identical keyimages";
      return false;
    }
  ...
  size_t core::get_blockchain_total_transactions() { return m_blockchain.getTotalTransactions(); }
  ...
  bool core::add_new_tx(const Transaction& tx, const Crypto::Hash& tx_hash, size_t blob_size,
                        tx_verification_context& tvc, bool keeped_by_block, uint32_t height)
  {
    // Locking on m_mempool and m_blockchain closes possibility to add tx to memory pool which is
    ...
    {
      logger(TRACE) << "<< Core.cpp << "
                    << "tx " << tx_hash << " is already in blockchain";
      return true;
    }
    ...
    {
      logger(TRACE) << "<< Core.cpp << "
                    << "tx " << tx_hash << " is already in transaction pool";
      return true;
    }
                                ...
                                uint32_t& height, const BinaryArray& ex_nonce)
  {
...

    {
      ...
      if (height >= m_currency.timestampCheckWindow())
      {
        ...
        for (uint32_t offset = height - static_cast<uint32_t>(m_currency.timestampCheckWindow());
    ...
    if (!m_mempool.fill_block_template(b, median_size, m_currency.maxBlockCumulativeSize(height),
                                       already_generated_coins, txs_size, fee, height))
    {
      return false;
    }
    ...
    bool r = m_currency.constructMinerTx(height, median_size, already_generated_coins, txs_size,
                                         fee, adr, b.baseTransaction, ex_nonce, 11);
    if (!r)
    {
      logger(ERROR, BRIGHT_RED) << "<< Core.cpp << "
                                << "Failed to construct miner tx, first chance";
      return false;
    }
    ...
    {
      r = m_currency.constructMinerTx(height, median_size, already_generated_coins, cumulative_size,
                                      fee, adr, b.baseTransaction, ex_nonce, 11);
      ...
      if (coinbase_blob_size < cumulative_size - txs_size)
      {
        ...
        // here  could be 1 byte difference, because of extra field counter is varint, and it can
        // become from 1-byte len to 2-bytes len.
        if (cumulative_size != txs_size + getObjectBinarySize(b.baseTransaction))
        {
          ...
          if (cumulative_size != txs_size + getObjectBinarySize(b.baseTransaction))
          {
            // fuck, not lucky, -1 makes varint-counter size smaller, in that case we continue to
            // grow with cumulative_size
          ...
          logger(DEBUGGING, BRIGHT_GREEN)
              << "Setting extra for block: " << b.baseTransaction.extra.size() << "<< Core.cpp << "
              << ", try_count=" << try_count;
        }
      }
  ...
  bool core::saveBlockchain() { return m_blockchain.storeCache(); }
                                        ...
                                        bool relay_block)
  {
    ...
    {
      logger(INFO) << "<< Core.cpp << "
                   << "WRONG BLOCK BLOB, too big size " << block_blob.size() << "<< Core.cpp << "
                   << ", rejected";
                                   ...
                                   bool control_miner, bool relay_block)
  {
    ...
    if (relay_block && bvc.m_added_to_main_chain)
    {
      ...
      {
        logger(INFO) << "<< Core.cpp << "
                     << "Block added, but it seems that reorganize just happened after that, do "
                        "not relay this block";
      }
      else
      {
        if (!(txs.size() == b.transactionHashes.size() && missed_txs.empty()))
        {
          logger(ERROR, BRIGHT_RED)
              ...
              << "<< Core.cpp << "
              << " txs.size()=" << txs.size() << "<< Core.cpp << "
              << ", b.transactionHashes.size()=" << b.transactionHashes.size() << "<< Core.cpp << "
  ...
  size_t core::get_pool_transactions_count() { return m_mempool.get_transactions_count(); }

  bool core::have_block(const Crypto::Hash& id) { return m_blockchain.haveBlock(id); }

  bool core::parse_tx_from_blob(Transaction& tx, Crypto::Hash& tx_hash,
                                Crypto::Hash& tx_prefix_hash, const BinaryArray& blob)
  {
  ...
  // void core::get_all_known_block_ids(std::list<Crypto::Hash> &main, std::list<Crypto::Hash> &alt,
  // std::list<Crypto::Hash> &invalid) {
  //  m_blockchain.get_all_known_block_ids(main, alt, invalid);
  //}

  std::string core::print_pool(bool short_format) { return m_mempool.print_pool(short_format); }
  ...
  bool core::on_idle()
  {
    if (!m_starter_message_showed)
    {
      logger(INFO) << ENDL
                   << "**********************************************************************"
                   << ENDL
                   << "The daemon will continue synchronizing with the network. If this is a new "
                      "installation it may take up to several hours."
                   << ENDL
                   ...
                   << ENDL << "Use \"help\" command to see the list of available commands." << ENDL
  ...
  bool core::addObserver(ICoreObserver* observer) { return m_observerManager.add(observer); }

  bool core::removeObserver(ICoreObserver* observer) { return m_observerManager.remove(observer); }

  void core::blockchainUpdated() { m_observerManager.notify(&ICoreObserver::blockchainUpdated); }

  void core::txDeletedFromPool() { poolUpdated(); }

  void core::poolUpdated() { m_observerManager.notify(&ICoreObserver::poolUpdated); }
                                                        ...
                                                        uint32_t startFullOffset)
  {
    ...
    if (startOffset < startFullOffset)
    {
      result = lbs->getBlockIds(
          startOffset, std::min(static_cast<uint32_t>(BLOCKS_IDS_SYNCHRONIZING_DEFAULT_COUNT),
                                startFullOffset - startOffset));
    }
  ...
  bool core::getBlockReward(size_t medianSize, size_t currentBlockSize,
                            uint64_t alreadyGeneratedCoins, uint64_t fee, uint32_t height,
                            uint64_t& reward, int64_t& emissionChange)
  {
    return m_currency.getBlockReward(medianSize, currentBlockSize, alreadyGeneratedCoins, fee,
                                     height, reward, emissionChange);
  }
                                      ...
                                      std::list<std::pair<Crypto::Hash, size_t>>& outputReferences)
  {
    struct outputs_visitor
    {
      ...
      outputs_visitor(std::list<std::pair<Crypto::Hash, size_t>>& resultsCollector)
                                        ...
                                        std::vector<Transaction>& transactions)
  {
    ...
    blockchainTransactionHashes.insert(blockchainTransactionHashes.end(),
                                       poolTransactionHashes.begin(), poolTransactionHashes.end());
  ...
  uint64_t core::getNextBlockDifficulty() { return m_blockchain.getDifficultyForNextBlock(); }

  uint64_t core::getTotalGeneratedAmount() { return m_blockchain.getCoinsInCirculation(); }

  uint64_t core::fullDepositAmount() const { return m_blockchain.fullDepositAmount(); }
                                       ...
                                       bool keptByBlock, uint32_t height)
  {
    ...
    {
      logger(INFO) << "<< Core.cpp << "
                   << "WRONG TRANSACTION BLOB, Failed to check tx " << txHash
                   << " syntax, rejected";
      tvc.m_verification_failed = true;
      return false;
    }
    ...
    {
      logger(ERROR) << "<< Core.cpp << "
                    << "Transaction verification impossible: " << txHash;
    }

    if (tvc.m_added_to_pool)
    {
      logger(DEBUGGING) << "<< Core.cpp << "
                        << "tx added: " << txHash;
      poolUpdated();
    }
  ...
  std::unique_ptr<IBlock> core::getBlock(const Crypto::Hash& blockId)
  {
    ...
    {
      logger(DEBUGGING) << "<< Core.cpp << "
                        << "Can't find transactions for block: " << blockId;
      return std::unique_ptr<BlockWithTransactions>(nullptr);
    }
