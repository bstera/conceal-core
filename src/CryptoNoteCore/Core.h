               ...
               public ITxPoolObserver
  {
   public:
    ...
    virtual bool handle_incoming_tx(const BinaryArray& tx_blob, tx_verification_context& tvc,
        ...
        override;  // Deprecated. Should be removed with CryptoNoteProtocolHandler.
    bool handle_incoming_block_blob(const BinaryArray& block_blob, block_verification_context& bvc,
    ...
    virtual bool get_block_template(Block& b, const AccountPublicAddress& adr,
    ...
    virtual bool getBlockReward(size_t medianSize, size_t currentBlockSize,
    ...
    virtual bool getOutByMSigGIndex(uint64_t amount, uint64_t gindex,
                                    MultisignatureOutput& out) override;
    virtual std::unique_ptr<IBlock> getBlock(const Crypto::Hash& blocksId) override;
    virtual bool handleIncomingTransaction(const Transaction& tx, const Crypto::Hash& txHash,
                                           size_t blobSize, tx_verification_context& tvc,
                                           bool keptByBlock, uint32_t height) override;
    ...
    bool get_blocks(const t_ids_container& block_ids, t_blocks_container& blocks,
    ...
    virtual bool queryBlocks(const std::vector<Crypto::Hash>& block_ids, uint64_t timestamp,
                             ...
                             uint32_t& full_offset, std::vector<BlockFullInfo>& entries) override;
    ...
    virtual bool getTransaction(const Crypto::Hash& id, Transaction& tx,
    ...
    virtual bool get_tx_outputs_gindexs(const Crypto::Hash& tx_id,
   ...
   private:
    ...
    bool handle_incoming_block(const Block& b, block_verification_context& bvc, bool control_miner,
                               bool relay_block);
