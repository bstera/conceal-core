...
{
  bool parseAndValidateTransactionFromBinaryArray(const BinaryArray& tx_blob, Transaction& tx,
                                                  Hash& tx_hash, Hash& tx_prefix_hash)
  {
                                 ...
                                 size_t real_output_index, KeyPair& in_ephemeral, KeyImage& ki)
  {
    ...
    r = derive_public_key(recv_derivation, real_output_index, ack.address.spendPublicKey,
    ...
    derive_secret_key(recv_derivation, real_output_index, ack.spendSecretKey,
                      in_ephemeral.secretKey);
  ...
  bool constructTransaction(const AccountKeys& sender_account_keys,
                            ...
                            Logging::ILogger& log, Crypto::SecretKey& transactionSK)
  {
    ...
    for (const TransactionDestinationEntry& dst_entr : shuffled_dsts)
    {
      ...
      r = derive_public_key(derivation, output_index, dst_entr.addr.spendPublicKey,
                            out_eph_public_key);
      if (!(r))
      {
    ...
    {
      logger(ERROR) << "Transaction inputs money (" << summary_inputs_money
    ...
    for (const TransactionSourceEntry& src_entr : sources)
    {
      ...
      generate_ring_signature(tx_prefix_hash, boost::get<KeyInput>(tx.inputs[i]).keyImage,
  ...
  bool is_out_to_acc(const AccountKeys& acc, const KeyOutput& out_key,
                     const KeyDerivation& derivation, size_t keyIndex)
  {
    PublicKey pk;
  ...
  bool is_out_to_acc(const AccountKeys& acc, const KeyOutput& out_key, const PublicKey& tx_pub_key,
                     size_t keyIndex)
  {
    KeyDerivation derivation;
  ...
  {
    auto it =
        std::lower_bound(Currency::PRETTY_AMOUNTS.begin(), Currency::PRETTY_AMOUNTS.end(), amount);
