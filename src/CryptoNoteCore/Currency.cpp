...
namespace CryptoNote
{
  ...
  const std::vector<uint64_t> Currency::REWARD_INCREASING_FACTOR = {
    ...
    4500000,  4750000,  5000000,  5250000,  5500000,  5750000,  6000000,  6250000,  6500000,
    6750000,  7000000,  7250000,  7500000,  7750000,  8000000,  8250000,  8500000,  8750000,
    9000000,  9250000,  9500000,  9750000,  10000000, 10250000, 10500000, 10750000, 11000000,
    11250000, 11500000, 11750000, 12000000, 12250000, 12500000, 12750000, 13000000, 13250000,
    13500000, 13750000, 14000000, 14250000, 14500000, 14750000, 15000000
  };
      ...
      std::vector<difficulty_type> cumulative_difficulties) const
  {
    ...
    for (uint64_t i = 1; i <= N; i++)
    {
      ...
      // Limit solvetime ST to 6*T to prevent large drop in difficulty that could cause
      // oscillations.
  ...
  Transaction CurrencyBuilder::generateGenesisTransaction()
  {
    ...
    CryptoNote::AccountPublicAddress ac =
        boost::value_initialized<CryptoNote::AccountPublicAddress>();
