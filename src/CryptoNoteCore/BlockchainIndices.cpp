...
namespace CryptoNote
{
  ...
  void PaymentIdIndex::clear() { index.clear(); }

  void PaymentIdIndex::serialize(ISerializer& s) { s(index, "index"); }
  ...
  bool TimestampBlocksIndex::find(uint64_t timestampBegin, uint64_t timestampEnd,
                                  uint32_t hashesNumberLimit, std::vector<Crypto::Hash>& hashes,
  ...
  void TimestampBlocksIndex::clear() { index.clear(); }

  void TimestampBlocksIndex::serialize(ISerializer& s) { s(index, "index"); }
  ...
  bool TimestampTransactionsIndex::find(uint64_t timestampBegin, uint64_t timestampEnd,
                                        ...
                                        uint64_t& hashesNumberWithinTimestamps)
  {
    ...
    auto begin = index.lower_bound(timestampBegin);
    auto end = index.upper_bound(timestampEnd);
    if (timestampEnd == static_cast<uint64_t>(0) && end == begin && begin == index.begin() &&
        index.size() > 0)
  ...
  void TimestampTransactionsIndex::clear() { index.clear(); }

  void TimestampTransactionsIndex::serialize(ISerializer& s) { s(index, "index"); }
  ...
  bool GeneratedTransactionsIndex::add(const Block& block)
  {
    ...
    bool status =
        index.emplace(blockHeight, lastGeneratedTxNumber + block.transactionHashes.size() + 1)
            .second;  // Plus miner tx
    if (status)
    {
  ...
  void GeneratedTransactionsIndex::clear() { index.clear(); }
  ...
  void OrphanBlocksIndex::clear() { index.clear(); }
