...
namespace CryptoNote
{
  ...
  class Blockchain : public CryptoNote::ITransactionValidator
  {
   public:
    ...
    virtual bool checkTransactionInputs(const CryptoNote::Transaction &tx,
                                        BlockInfo &maxUsedBlock) override;
    ...
    bool scanOutputKeysForIndexes(const KeyInput &tx_in_to_key, visitor_t &vis,
                                  uint32_t *pmax_related_block_height = NULL);
   ...
   private:
    ...
    typedef parallel_flat_hash_map<uint64_t, std::vector<std::pair<TransactionIndex, uint16_t>>>
        outputs_container;  // Crypto::Hash - tx hash, size_t - index of out in transaction
    typedef parallel_flat_hash_map<uint64_t, std::vector<MultisignatureOutputUsage>>
        MultisignatureOutputsContainer;
    ...
    bool add_out_to_get_random_outs(
        ...
        COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_outs_for_amount &result_outs, uint64_t amount,
        size_t i);
    ...
    bool checkTransactionInputs(const Transaction &tx, const Crypto::Hash &tx_prefix_hash,
                                uint32_t *pmax_used_block_height = NULL);
  ...
  {
   public:
    LockedBlockchainStorage(Blockchain &bc) : m_bc(bc), m_lock(bc.m_blockchain_lock) { }
                                            ...
                                            uint32_t *pmax_related_block_height)
  {
    ...
    for (uint64_t i : absolute_offsets)
    {
      ...
      // if (!(tx_it != m_transactionMap.end())) { logger(ERROR, BRIGHT_RED) << "Wrong transaction
      // id in output indexes: " << Common::podToHex(amount_outs_vec[i].first); return false; }
      ...
      if (!(amount_outs_vec[i].second < tx.tx.outputs.size()))
      {
        logger(Logging::ERROR, Logging::BRIGHT_RED)
            ...
            << ", expected less then " << tx.tx.outputs.size();
        return false;
      }
