...
namespace CryptoNote
{
  ...
  void InvestmentIndex::reserve(DepositHeight expectedHeight) { index.reserve(expectedHeight + 1); }
  ...
  auto InvestmentIndex::size() const -> DepositHeight { return blockCount; }

  auto InvestmentIndex::upperBound(DepositHeight height) const -> IndexType::const_iterator
  {
    return std::upper_bound(index.cbegin(), index.cend(), height,
                            [](DepositHeight height, const InvestmentIndexEntry& left) {
                              return height < left.height;
                            });
  }
