  ...
  class BasicUpgradeDetector : public UpgradeDetectorBase
  {
   public:
        ...
        else
        {
          ...
          if (blockVersionAfterUpgradeHeight != m_targetVersion)
          {
            logger(Logging::ERROR, Logging::BRIGHT_RED)
                ...
                << " has invalid version " << blockVersionAfterUpgradeHeight << ", expected "
    ...
    void blockPushed()
    {
      ...
      else if (m_votingCompleteHeight != UNDEF_HEIGHT)
      {
        ...
        if (m_blockchain.size() <= upgradeHeight())
        {
          ...
          {
            auto interval =
                m_currency.difficultyTarget() * (upgradeHeight() - m_blockchain.size() + 2);
            ...
            logger(Logging::TRACE, Logging::BRIGHT_GREEN)
                ...
                << " at about " << upgradeTimeStr << " (in "
                << Common::timeIntervalToString(interval) << ")! Current last block index "
                << (m_blockchain.size() - 1) << ", hash " << get_block_hash(m_blockchain.back().bl);
          }
        }
    ...
    size_t getNumberOfVotes(uint32_t height)
    {
      ...
      for (size_t i = height + 1 - m_currency.upgradeVotingWindow(); i <= height; ++i)
      {
        ...
        voteCounter +=
            (b.majorVersion == m_targetVersion - 1) && (b.minorVersion == BLOCK_MINOR_VERSION_1)
                ? 1
                : 0;
      }
   ...
   private:
    ...
    bool isVotingComplete(uint32_t height)
    {
      ...
      return m_currency.upgradeVotingThreshold() * m_currency.upgradeVotingWindow() <=
             100 * voteCounter;
    }
