...
namespace CryptoNote
{
  ...
  bool Checkpoints::add_checkpoint(uint32_t height, const std::string &hash_str)
  {
    ...
    {
      logger(ERROR) << "<< Checkpoints.cpp << "
                    << "Incorrect hash in checkpoints";
      return false;
    }
  ...
  bool Checkpoints::check_block(uint32_t height, const Crypto::Hash &h, bool &is_a_checkpoint) const
  {
    ...
    {
      logger(Logging::ERROR) << "<< Checkpoints.cpp << "
                                                 ...
                                                 uint32_t block_height) const
  {
    ...
    if (block_height <
            blockchain_height - CryptoNote::parameters::CRYPTONOTE_MINED_MONEY_UNLOCK_WINDOW &&
        !is_in_checkpoint_zone(block_height))
    {
      logger(Logging::DEBUGGING, Logging::WHITE)
          << "<< Checkpoints.cpp << "
          << "Reorganization depth too deep : " << (blockchain_height - block_height)
          << ". Block Rejected";
      return false;
    }
  ...
  bool Checkpoints::load_checkpoints_from_dns()
  {
    ...
    logger(Logging::DEBUGGING) << "<< Checkpoints.cpp << "
                               << "Fetching DNS checkpoint records from " << domain;

    if (!Common::fetch_dns_txt(domain, records))
    {
      logger(Logging::DEBUGGING) << "<< Checkpoints.cpp << "
                                 << "Failed to lookup DNS checkpoint records from " << domain;
    }
    ...
    for (const auto &record : records)
    {
      ...
      {
        logger(Logging::INFO) << "<< Checkpoints.cpp << "
                              << "Failed to parse DNS checkpoint record: " << record;
        continue;
      }

      if (!(0 == m_points.count(height)))
      {
        logger(DEBUGGING) << "<< Checkpoints.cpp << "
                          << "Checkpoint already exists for height: " << height
                          << ". Ignoring DNS checkpoint.";
      }
      else
      {
        add_checkpoint(height, hash_str);
        logger(INFO) << "<< Checkpoints.cpp << "
                     << "Added DNS checkpoint: " << height_str << ":" << hash_str;
      }
    }
