...
namespace
{
                       ...
                       CryptoNote::TransactionInput& in)
  {
    switch (tag)
    {
      ...
      default: throw std::runtime_error("Unknown variant tag");
    }
  }
...
namespace Crypto
{
  ...
  bool serialize(EllipticCurveScalar& ecScalar, Common::StringView name,
                 CryptoNote::ISerializer& serializer)
  {
    return serializePod(ecScalar, name, serializer);
  }

  bool serialize(EllipticCurvePoint& ecPoint, Common::StringView name,
                 CryptoNote::ISerializer& serializer)
  {
