...
namespace PaymentService
{
  ...
  {
    desc.add_options()("bind-address", po::value<std::string>()->default_value("0.0.0.0"),
                       "payment service bind address")(
        "bind-port", po::value<uint16_t>()->default_value(8070), "payment service bind port")(
        "rpc-user", po::value<std::string>()->default_value(""),
        "username to use the payment service. If authorization is not required, leave it empty")(
        "rpc-password", po::value<std::string>()->default_value(""),
        "password to use the payment service. If authorization is not required, leave it empty")(
        "container-file,w", po::value<std::string>(), "container file")(
        "container-password,p", po::value<std::string>(), "container password")(
        "generate-container,g", "generate new container file with one wallet and exit")(
        "view-key", po::value<std::string>(),
        "Generate a wallet container with this secret view <key>")(
        "spend-key", po::value<std::string>(),
        ...
        "server root. The service will use it as working directory. Don't set it if don't want to "
        "change it")("log-level", po::value<size_t>(), "log level")(
  ...
  void Configuration::init(const boost::program_options::variables_map& options)
  {
    ...
    if (registerService && unregisterService)
    {
      throw ConfigurationError(
          "It's impossible to use both \"register-service\" and \"unregister-service\" at the same "
          "time");
    }
