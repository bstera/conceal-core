...
#include "random.h"
  ...
  bool crypto_ops::generate_key_derivation(const PublicKey &key1, const SecretKey &key2,
  ...
  static void derivation_to_scalar(const KeyDerivation &derivation, size_t output_index,
  ...
  bool crypto_ops::derive_public_key(const KeyDerivation &derivation, size_t output_index,
                                     const PublicKey &base, const uint8_t *suffix,
                                     size_t suffixLength, PublicKey &derived_key)
  {
    EllipticCurveScalar scalar;
    ge_p3 point1;
    ge_p3 point2;
  ...
  bool crypto_ops::underive_public_key_and_get_scalar(const KeyDerivation &derivation,
                                                      size_t output_index,
  ...
  void crypto_ops::derive_secret_key(const KeyDerivation &derivation, size_t output_index,
                                     const SecretKey &base, const uint8_t *suffix,
                                     size_t suffixLength, SecretKey &derived_key)
  {
    EllipticCurveScalar scalar;
    assert(sc_check(reinterpret_cast<const unsigned char *>(&base)) == 0);
    derivation_to_scalar(derivation, output_index, suffix, suffixLength, scalar);
  ...
  bool crypto_ops::underive_public_key(const KeyDerivation &derivation, size_t output_index,
                                       const PublicKey &derived_key, const uint8_t *suffix,
                                       size_t suffixLength, PublicKey &base)
  {
    EllipticCurveScalar scalar;
    ge_p3 point1;
    ge_p3 point2;
  ...
  void crypto_ops::generate_tx_proof(const Hash &prefix_hash, const PublicKey &R,
                                     const PublicKey &A, const PublicKey &D, const SecretKey &r,
                                     Signature &sig)
  {
    // sanity check
    ge_p3 R_p3;
  ...
  bool crypto_ops::check_tx_proof(const Hash &prefix_hash, const PublicKey &R, const PublicKey &A,
  ...
  void crypto_ops::generate_incomplete_key_image(const PublicKey &pub,
                                                 EllipticCurvePoint &incomplete_key_image)
  {
    ge_p3 point;
    hash_to_ec(pub, point);
...
#pragma warning(disable : 4200)
                                           ...
                                           const SecretKey &sec, size_t sec_index, Signature *sig)
  {
    ...
    for (i = 0; i < pubs_count; i++)
    {
      ...
      else
      {
        ...
        ge_double_scalarmult_precomp_vartime(&tmp2, reinterpret_cast<unsigned char *>(&sig[i]) + 32,
                                             &tmp3, reinterpret_cast<unsigned char *>(&sig[i]),
                                             image_pre);
        ge_tobytes(reinterpret_cast<unsigned char *>(&buf->ab[i].b), &tmp2);
    ...
    sc_sub(reinterpret_cast<unsigned char *>(&sig[sec_index]),
                                        ...
                                        const Signature *sig)
  {
    ...
    for (i = 0; i < pubs_count; i++)
    {
      ...
      ge_double_scalarmult_precomp_vartime(
          &tmp2, reinterpret_cast<const unsigned char *>(&sig[i]) + 32, &tmp3,
