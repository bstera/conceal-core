...
namespace Crypto
{
  ...
  {
    if (hw_check_aes())
      cryptonight_hash<false, CRYPTONIGHT_FAST_V8>(data, length, reinterpret_cast<char *>(&hash),
                                                   context);
    else
      cryptonight_hash<true, CRYPTONIGHT_FAST_V8>(data, length, reinterpret_cast<char *>(&hash),
                                                  context);
  }
  ...
  {
    if (hw_check_aes())
      cryptonight_hash<false, CRYPTONIGHT_CONCEAL>(data, length, reinterpret_cast<char *>(&hash),
                                                   context);
    else
      cryptonight_hash<true, CRYPTONIGHT_CONCEAL>(data, length, reinterpret_cast<char *>(&hash),
                                                  context);
  }
