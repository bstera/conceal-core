...
namespace
{
  ...
  uint64_t calculateInvestmentsAmount(
      const std::vector<CryptoNote::TransactionOutputInformation>& transfers,
      const CryptoNote::Currency& currency, const std::vector<uint32_t> heights)
  {
    int index = 0;
...
namespace CryptoNote
{
  ...
  void WalletLegacy::initSync()
  {
    ...
    m_sender.reset(new WalletTransactionSender(
        m_currency, m_transactionsCache, m_account.getAccountKeys(), *m_transferDetails, m_node));
    m_state = INITIALIZED;
  ...
  void WalletLegacy::doLoad(std::istream& source)
  {
    ...
    try
    {
      ...
      for (auto& o : allTransfers)
      {
        ...
        {
          m_transfersSync.addPublicKeysSeen(m_account.getAccountKeys().address, o.transactionHash,
                                            o.outputKey);
        }
      }
    }
  ...
  std::error_code WalletLegacy::changePassword(const std::string& oldPassword,
  ...
  bool WalletLegacy::getTransaction(TransactionId transactionId,
  ...
  TransactionId WalletLegacy::sendTransaction(Crypto::SecretKey& transactionSK,
                                              const WalletLegacyTransfer& transfer, uint64_t fee,
                                              const std::string& extra, uint64_t mixIn,
                                              ...
                                              uint64_t ttl)
  {
    ...
    return sendTransaction(transactionSK, transfers, fee, extra, mixIn, unlockTimestamp, messages,
                           ttl);
  }
  ...
  TransactionId WalletLegacy::sendTransaction(Crypto::SecretKey& transactionSK,
                                              ...
                                              uint64_t ttl)
  {
    /* Regular transaction fees should be at least 1000 X as of Consensus 2019. In this case we also
       check to ensure that it is not a self-destructive message, which will have a TTL that is
       larger than 0 */
    if (ttl == 0)
    {
    ...
    {
      std::unique_lock<std::mutex> lock(m_cacheMutex);
      request = m_sender->makeSendRequest(transactionSK, optimize, txId, events, transfers, fee,
                                          extra, mixIn, unlockTimestamp, messages, ttl);
    }
    ...
    {
      m_asyncContextCounter.addAsyncContext();
      request->perform(m_node, std::bind(&WalletLegacy::sendTransactionCallback, this,
      ...
      uint64_t threshold, size_t minInputCount, size_t maxInputCount)
  {
    ...
    std::copy(trimmedSelectedOuts.begin(), trimmedSelectedOuts.end(),
              std::back_inserter(selectedOutputs));
    return selectedOutputs;
  }
      ...
      const std::string& extra, uint64_t mixIn, uint64_t unlockTimestamp)
  {
    ...
    {
      std::unique_lock<std::mutex> lock(m_cacheMutex);
      request = m_sender->makeSendFusionRequest(txId, events, transfers, fusionInputs, fee, extra,
                                                0, unlockTimestamp);
    }
  ...
  void WalletLegacy::sendTransactionCallback(WalletRequest::Callback callback, std::error_code ec)
  {
    ...
    {
      m_asyncContextCounter.addAsyncContext();
      nextRequest->perform(m_node, std::bind(&WalletLegacy::synchronizationCallback, this,
                                             std::placeholders::_1, std::placeholders::_2));
    }
  }
  ...
  void WalletLegacy::synchronizationProgressUpdated(uint32_t current, uint32_t total)
  {
    ...
    m_observerManager.notify(&IWalletLegacyObserver::synchronizationProgressUpdated, current,
                             total);
  ...
  void WalletLegacy::synchronizationCompleted(std::error_code result)
  {
    ...
    auto deletedTransactions = deleteOutdatedUnconfirmedTransactions();
    std::for_each(
        deletedTransactions.begin(), deletedTransactions.end(), [&](TransactionId transactionId) {
          m_observerManager.notify(&IWalletLegacyObserver::transactionUpdated, transactionId);
        });
  ...
  void WalletLegacy::onTransactionUpdated(ITransfersSubscription* object,
                                          const Hash& transactionHash)
  {
    ...
    uint64_t amountIn;
    uint64_t amountOut;
    if (m_transferDetails->getTransactionInformation(transactionHash, txInfo, &amountIn,
                                                     &amountOut))
    {
      std::unique_lock<std::mutex> lock(m_cacheMutex);

      auto newDepositOuts = m_transferDetails->getTransactionOutputs(
          transactionHash,
          ITransfersContainer::IncludeTypeDeposit | ITransfersContainer::IncludeStateAll);
      auto spentDeposits = m_transferDetails->getTransactionInputs(
          transactionHash, ITransfersContainer::IncludeTypeDeposit);

      events = m_transactionsCache.onTransactionUpdated(
          txInfo, static_cast<int64_t>(amountOut) - static_cast<int64_t>(amountIn), newDepositOuts,
          spentDeposits, m_currency);
  ...
  void WalletLegacy::onTransactionDeleted(ITransfersSubscription* object,
                                          const Hash& transactionHash)
  {
    std::deque<std::unique_ptr<WalletLegacyEvent>> events;

    {
      ...
      std::unique_ptr<WalletLegacyEvent> actualDepositBalanceUpdated =
      ...
      std::unique_ptr<WalletLegacyEvent> pendingDepositBalanceUpdated =
          getPendingDepositBalanceChangedEvent();
      if (pendingDepositBalanceUpdated)
      {
        events.push_back(std::move(pendingDepositBalanceUpdated));
      }
  ...
  void WalletLegacy::onTransfersUnlocked(
      ITransfersSubscription* object,
      const std::vector<TransactionOutputInformation>& unlockedTransfers)
  {
    std::unique_lock<std::mutex> lock(m_cacheMutex);
  ...
  void WalletLegacy::onTransfersLocked(
      ITransfersSubscription* object,
      const std::vector<TransactionOutputInformation>& lockedTransfers)
  {
    std::unique_lock<std::mutex> lock(m_cacheMutex);
  ...
  std::unique_ptr<WalletLegacyEvent> WalletLegacy::getActualDepositBalanceChangedEvent()
  {
    ...
    {
      event =
          std::unique_ptr<WalletLegacyEvent>(new WalletActualDepositBalanceUpdatedEvent(actual));
    }
  ...
  std::unique_ptr<WalletLegacyEvent> WalletLegacy::getPendingDepositBalanceChangedEvent()
  {
    ...
    {
      event =
          std::unique_ptr<WalletLegacyEvent>(new WalletPendingDepositBalanceUpdatedEvent(pending));
    }
  ...
  std::unique_ptr<WalletLegacyEvent> WalletLegacy::getActualInvestmentBalanceChangedEvent()
  {
    ...
    {
      event =
          std::unique_ptr<WalletLegacyEvent>(new WalletActualInvestmentBalanceUpdatedEvent(actual));
    }
  ...
  std::unique_ptr<WalletLegacyEvent> WalletLegacy::getPendingInvestmentBalanceChangedEvent()
  {
    ...
    if (pending != prevPending)
    {
      event = std::unique_ptr<WalletLegacyEvent>(
          new WalletPendingInvestmentBalanceUpdatedEvent(pending));
    }
  ...
  uint64_t WalletLegacy::calculateActualDepositBalance()
  {
    ...
    return calculateDepositsAmount(transfers, m_currency, heights) -
           m_transactionsCache.countUnconfirmedSpentDepositsTotalAmount();
  }
  ...
  std::vector<uint32_t> WalletLegacy::getTransactionHeights(
      const std::vector<TransactionOutputInformation> transfers)
  {
  ...
  void WalletLegacy::pushBalanceUpdatedEvents(
      std::deque<std::unique_ptr<WalletLegacyEvent>>& eventsQueue)
  {
    auto actualDepositBalanceUpdated = getActualDepositBalanceChangedEvent();
  ...
  bool WalletLegacy::getTxProof(Crypto::Hash& txid, CryptoNote::AccountPublicAddress& address,
                                Crypto::SecretKey& tx_key, std::string& sig_str)
  {
  ...
  bool compareTransactionOutputInformationByAmount(const TransactionOutputInformation& a,
  ...
  std::string WalletLegacy::getReserveProof(const uint64_t& reserve, const std::string& message)
  {
    ...
    // minimize the number of outputs included in the proof, by only picking the N largest outputs
    // that can cover the requested min reserve amount
    ...
    for (size_t i = 0; i < selected_transfers.size(); ++i)
    {
      ...
      Crypto::KeyImage ki;
      const bool r =
          CryptoNote::generate_key_image_helper(m_account.getAccountKeys(), td.transactionPublicKey,
                                                td.outputInTransaction, ephemeral, ki);
      if (!r)
      {
    ...
    for (size_t i = 0; i < selected_transfers.size(); ++i)
    {
      ...
      {
        Crypto::KeyImage sk =
            Crypto::scalarmultKey(*reinterpret_cast<const Crypto::KeyImage*>(&txPubKey),
      ...
      Crypto::generate_tx_proof(prefix_hash, keys.address.viewPublicKey, txPubKey,
      ...
      Crypto::generate_ring_signature(prefix_hash, proof.key_image, &pubs[0], 1,
                                      ephemeral.secretKey, 0, &proof.key_image_sig);
    }
    // generate signature for the spend key that received those outputs
    Crypto::Signature signature;
    Crypto::generate_signature(prefix_hash, keys.address.spendPublicKey, keys.spendSecretKey,
                               signature);
