...
namespace CryptoNote
{
  ...
  {
   public:
    typedef std::function<void(std::deque<std::unique_ptr<WalletLegacyEvent>>&,
                               std::unique_ptr<WalletRequest>&, std::error_code)>
        Callback;

    virtual ~WalletRequest(){};

    virtual void perform(INode& node,
  ...
  class WalletGetRandomOutsByAmountsRequest : public WalletRequest
  {
   public:
    ...
    {
      node.getRandomOutsByAmounts(std::move(m_amounts), m_outsCount, std::ref(m_context->outs),
  ...
  {
   public:
    WalletRelayDepositTransactionRequest(const Transaction& tx, Callback cb)
        : m_tx(tx), m_cb(cb) { }
