...
namespace CryptoNote
{
  ...
  namespace
  {
    class DepositIdSequenceIterator
        : public std::iterator<std::random_access_iterator_tag, DepositId>
    {
     public:
      ...
      const DepositIdSequenceIterator& operator++()
      {
        ++val;
        return *this;
      }
      DepositIdSequenceIterator operator++(int)
      {
        DepositIdSequenceIterator copy(*this);
        ++val;
        return copy;
      }
      ...
      DepositIdSequenceIterator operator+(const difference_type& n) const
      {
        return DepositIdSequenceIterator(val + n);
      }
      DepositIdSequenceIterator operator-(const difference_type& n) const
      {
        return DepositIdSequenceIterator(val - n);
      ...
      difference_type operator-(const DepositIdSequenceIterator& other) const
      {
        return val - other.val;
      }

      DepositIdSequenceIterator& operator+=(const difference_type& n)
      {
        val += n;
        return *this;
      }
    ...
    void convertLegacyDeposits(const std::vector<LegacyDepositInfo>& legacyDeposits,
  ...
  bool paymentIdIsSet(const PaymentId& paymentId) { return paymentId != NULL_HASH; }
  ...
  void WalletUserTransactionsCache::popFromPaymentsIndex(const PaymentId& paymentId,
                                                         Offset distance)
  {
  ...
  size_t WalletUserTransactionsCache::getTransactionCount() const { return m_transactions.size(); }

  size_t WalletUserTransactionsCache::getTransferCount() const { return m_transfers.size(); }

  size_t WalletUserTransactionsCache::getDepositCount() const { return m_deposits.size(); }

  TransactionId WalletUserTransactionsCache::addNewTransaction(
      uint64_t amount, uint64_t fee, const std::string& extra,
  ...
  void WalletUserTransactionsCache::updateTransactionSendingState(TransactionId transactionId,
                                                                  std::error_code ec)
  {
    auto& txInfo = m_transactions.at(transactionId);
    if (ec)
  ...
  std::deque<std::unique_ptr<WalletLegacyEvent>> WalletUserTransactionsCache::onTransactionUpdated(
      const TransactionInformation& txInfo, int64_t txBalance,
    ...
    if (id == CryptoNote::WALLET_LEGACY_INVALID_TRANSACTION_ID)
    {
      ...
      else
      {
        transaction.fee = txInfo.totalAmountIn < txInfo.totalAmountOut
                              ...
                              : txInfo.totalAmountIn - txInfo.totalAmountOut;
      }
      ...
      events.push_back(
          std::unique_ptr<WalletLegacyEvent>(new WalletExternalTransactionCreatedEvent(id)));
      auto updatedDepositIds =
          createNewDeposits(id, newDepositOutputs, currency, transaction.blockHeight);
      ...
      updatedDepositIds.insert(updatedDepositIds.end(), spentDepositIds.begin(),
                               spentDepositIds.end());

      if (!updatedDepositIds.empty())
      {
        events.push_back(std::unique_ptr<WalletLegacyEvent>(
            new WalletDepositsUpdatedEvent(std::move(updatedDepositIds))));
      }
    }
    else
    {
  ...
  std::deque<std::unique_ptr<WalletLegacyEvent>> WalletUserTransactionsCache::onTransactionDeleted(
      const Crypto::Hash& transactionHash)
  {
    ...
    if (m_unconfirmedTransactions.findTransactionId(transactionHash, id))
    {
      ...
      // LOG_ERROR("Unconfirmed transaction is deleted: id = " << id << ", hash = " <<
      // transactionHash);
      assert(false);
    }
    else
    ...
    if (id != CryptoNote::WALLET_LEGACY_INVALID_TRANSACTION_ID)
    {
      ...
      if (depositIdSequenceStart != depositIdSequenceEnd || !unspentDeposits.empty())
      {
        ...
        events.push_back(std::unique_ptr<WalletLegacyEvent>(
            new WalletDepositsUpdatedEvent(std::move(unspentDeposits))));
      }
    }
    else
    {
  ...
  std::vector<DepositId> WalletUserTransactionsCache::unlockDeposits(
      const std::vector<TransactionOutputInformation>& transfers)
  {
    ...
    {
      auto it = m_transactionOutputToDepositIndex.find(
          std::tie(transfer.transactionHash, transfer.outputInTransaction));
  ...
  std::vector<DepositId> WalletUserTransactionsCache::lockDeposits(
      const std::vector<TransactionOutputInformation>& transfers)
  {
  ...
  TransactionId WalletUserTransactionsCache::findTransactionByTransferId(
  ...
  bool WalletUserTransactionsCache::getTransfer(TransferId transferId,
  ...
  TransactionId WalletUserTransactionsCache::insertTransaction(
      WalletLegacyTransaction&& Transaction)
  {
  ...
  TransactionId WalletUserTransactionsCache::findTransactionByHash(const Hash& hash)
  {
    auto it = std::find_if(m_transactions.begin(), m_transactions.end(),
                           [&hash](const WalletLegacyTransaction& tx) { return tx.hash == hash; });
  ...
  DepositId WalletUserTransactionsCache::insertDeposit(const Deposit& deposit,
                                                       ...
                                                       const Hash& transactionHash)
  {
    ...
    m_transactionOutputToDepositIndex.emplace(
        std::piecewise_construct,
        std::forward_as_tuple(transactionHash, static_cast<uint32_t>(depositIndexInTransaction)),
        std::forward_as_tuple(id));

    return id;
  }

  bool WalletUserTransactionsCache::getDepositInTransactionInfo(DepositId depositId,
  ...
  std::vector<DepositId> WalletUserTransactionsCache::createNewDeposits(
      TransactionId creatingTransactionId,
      const std::vector<TransactionOutputInformation>& depositOutputs, const Currency& currency,
      uint32_t height)
  {
  ...
  DepositId WalletUserTransactionsCache::insertNewDeposit(
      const TransactionOutputInformation& depositOutput, TransactionId creatingTransactionId,
      const Currency& currency, uint32_t height)
  {
    ...
    assert(m_transactionOutputToDepositIndex.find(
               std::tie(depositOutput.transactionHash, depositOutput.outputInTransaction)) ==
           m_transactionOutputToDepositIndex.end());
  ...
  std::vector<DepositId> WalletUserTransactionsCache::processSpentDeposits(
      TransactionId spendingTransactionId,
      const std::vector<TransactionOutputInformation>& spentDepositOutputs)
  {
    std::vector<DepositId> deposits;
    ...
    for (size_t i = 0; i < spentDepositOutputs.size(); i++)
    {
      auto depositId = getDepositId(spentDepositOutputs[i].transactionHash,
                                    spentDepositOutputs[i].outputInTransaction);
  ...
  DepositId WalletUserTransactionsCache::getDepositId(const Hash& creatingTransactionHash,
                                                      uint32_t outputInTransaction)
  {
    auto it = m_transactionOutputToDepositIndex.find(
        std::tie(creatingTransactionHash, outputInTransaction));
    if (it == m_transactionOutputToDepositIndex.end())
    {
  ...
  std::vector<DepositId> WalletUserTransactionsCache::getDepositIdsBySpendingTransaction(
  ...
  void WalletUserTransactionsCache::addDepositSpendingTransaction(
      const Hash& transactionHash, const UnconfirmedSpentDepositDetails& details)
  {
