...
namespace CryptoNote
{
  ...
  class WalletUserTransactionsCache
  {
   public:
    ...
    void updateTransaction(TransactionId transactionId, const CryptoNote::Transaction& tx,
                           uint64_t amount,
    ...
    void addDepositSpendingTransaction(const Crypto::Hash& transactionHash,
                                       const UnconfirmedSpentDepositDetails& details);

    std::deque<std::unique_ptr<WalletLegacyEvent>> onTransactionUpdated(
        const TransactionInformation& txInfo, int64_t txBalance,
        const std::vector<TransactionOutputInformation>& newDeposits,
        const std::vector<TransactionOutputInformation>& spentDeposits, const Currency& currency);
    std::deque<std::unique_ptr<WalletLegacyEvent>> onTransactionDeleted(
        const Crypto::Hash& transactionHash);

    std::vector<DepositId> unlockDeposits(
    ...
    DepositId insertDeposit(const Deposit& deposit, size_t depositIndexInTransaction,
                            const Crypto::Hash& transactionHash);
    bool getDepositInTransactionInfo(DepositId depositId, Crypto::Hash& transactionHash,
                                     uint32_t& outputInTransaction);

    std::vector<Payments> getTransactionsByPaymentIds(
   ...
   private:
    ...
    std::vector<DepositId> createNewDeposits(
    ...
    DepositId insertNewDeposit(const TransactionOutputInformation& depositOutput,
                               ...
                               uint32_t height);
    std::vector<DepositId> processSpentDeposits(
        TransactionId spendingTransactionId,
        const std::vector<TransactionOutputInformation>& spentDepositOutputs);
    DepositId getDepositId(const Crypto::Hash& creatingTransactionHash,
                           uint32_t outputInTransaction);
    ...
    using UserPaymentIndex =
        std::unordered_map<PaymentId, std::vector<Offset>, boost::hash<PaymentId>>;

    void rebuildPaymentsIndex();
    void pushToPaymentsIndex(const PaymentId& paymentId, Offset distance);
    void pushToPaymentsIndexInternal(Offset distance, const WalletLegacyTransaction& info,
    ...
    std::unordered_map<std::tuple<Crypto::Hash, uint32_t>, DepositId>
        m_transactionOutputToDepositIndex;
