...
namespace CryptoNote
{
  ...
  std::unordered_map<std::string, RpcServer::RpcHandler<RpcServer::HandlerFunction>>
      RpcServer::s_handlers = {
        ...
        { "/get_o_indexes.bin",
          { binMethod<COMMAND_RPC_GET_TX_GLOBAL_OUTPUTS_INDEXES>(&RpcServer::on_get_indexes),
            false } },
        { "/getrandom_outs.bin",
  ...
  bool RpcServer::k_on_check_tx_proof(const K_COMMAND_RPC_CHECK_TX_PROOF::request& req,
                                      K_COMMAND_RPC_CHECK_TX_PROOF::response& res)
  {
    ...
    const size_t sig_len =
        Tools::Base58::encode(std::string((const char*)&sig, sizeof(Crypto::Signature))).size();
  ...
  bool RpcServer::k_on_check_reserve_proof(const K_COMMAND_RPC_CHECK_RESERVE_PROOF::request& req,
                                           K_COMMAND_RPC_CHECK_RESERVE_PROOF::response& res)
  {
    ...
    for (size_t i = 0; i < proofs.size(); ++i)
    {
      ...
      if (!Crypto::check_tx_proof(prefix_hash, address.viewPublicKey, txPubKey, proof.shared_secret,
                                  proof.shared_secret_sig))
      {
        // throw JsonRpc::JsonRpcError{ CORE_RPC_ERROR_CODE_INTERNAL_ERROR, "Failed to check
        // singature for shared secret" };
        res.good = false;
        return true;
      }
  ...
  bool RpcServer::on_query_blocks(const COMMAND_RPC_QUERY_BLOCKS::request& req,
  ...
  bool RpcServer::setViewKey(const std::string& view_key)
  {
    ...
    {
      logger(INFO) << "<< rpcserver.cpp << "
                   << "Could not parse private view key";
      return false;
    }
  ...
  bool RpcServer::on_get_info(const COMMAND_RPC_GET_INFO::request& req,
                              COMMAND_RPC_GET_INFO::response& res)
  {
    ...
    uint32_t last_block_height =
        boost::get<BaseInput>(blk.baseTransaction.inputs.front()).blockIndex;
  ...
  bool RpcServer::on_get_height(const COMMAND_RPC_GET_HEIGHT::request& req,
  ...
  bool RpcServer::on_send_raw_tx(const COMMAND_RPC_SEND_RAW_TX::request& req,
                                 COMMAND_RPC_SEND_RAW_TX::response& res)
  {
    ...
    if (!m_fee_address.empty() && m_view_key != NULL_SECRET_KEY) {
      if (!remotenode_check_incoming_tx(tx_blob)) {
        logger(INFO) << "<< rpcserver.cpp << " << "Transaction not relayed due to lack of remote
    node fee"; res.status = "Not relayed due to lack of node fee"; return true;
      }
    }
  ...
  bool RpcServer::on_start_mining(const COMMAND_RPC_START_MINING::request& req,
  ...
  bool RpcServer::remotenode_check_incoming_tx(const BinaryArray& tx_blob) {
          ...
          if (!CryptoNote::findOutputsToAccount(transaction, m_fee_acc, m_view_key, out, amount)) {
                  logger(INFO) << "<< rpcserver.cpp << " << "Could not find outputs to remote node
  fee address"; return false;
          }

          if (amount != 0) {
                  logger(INFO) << "<< rpcserver.cpp << " << "Node received relayed transaction fee:
  " << m_core.currency().formatAmount(amount) << " KRB"; return true;
          }
          return false;
  }
  ...
  */
  ...
  bool RpcServer::on_stop_mining(const COMMAND_RPC_STOP_MINING::request& req,
  ...
  bool RpcServer::on_stop_daemon(const COMMAND_RPC_STOP_DAEMON::request& req,
                                  ...
                                  F_COMMAND_RPC_GET_BLOCK_DETAILS::response& res)
  {
    ...
    size_t blockGrantedFullRewardZone =
        penalizeFee ? m_core.currency().blockGrantedFullRewardZone() :
                    // m_core.currency().blockGrantedFullRewardZoneV1();
            res.block.effectiveSizeMedian =
                std::max(res.block.sizeMedian, blockGrantedFullRewardZone);

    // virtual bool getBlockReward(size_t medianSize, size_t currentBlockSize, uint64_t
    // alreadyGeneratedCoins, uint64_t fee, uint32_t height, uint64_t& reward, int64_t&
    // emissionChange) = 0;

    if (!m_core.getBlockReward(res.block.sizeMedian, 0, prevBlockGeneratedCoins, 0,
                               res.block.height, maxReward, emissionChange))
    {
      return false;
    }
    if (!m_core.getBlockReward(res.block.sizeMedian, res.block.transactionsCumulativeSize,
                               prevBlockGeneratedCoins, 0, res.block.height, currentReward,
                               emissionChange))
    {
      return false;
    }

    // if (!m_core.getBlockReward(res.block.sizeMedian, 0, prevBlockGeneratedCoins, 0, penalizeFee,
    // maxReward, emissionChange)) {
    //   return false;
    // }
    // if (!m_core.getBlockReward(res.block.sizeMedian, res.block.transactionsCumulativeSize,
    // prevBlockGeneratedCoins, 0, penalizeFee, currentReward, emissionChange)) {
    //   return false;
    // }
    ...
    for (const Transaction& tx : txs)
    {
      ...
      transaction_short.fee =
          amount_in < amount_out + parameters::MINIMUM_FEE  // account for interest in output, it
                                                            // always has minimum fee
  ...
  bool RpcServer::on_getblockhash(const COMMAND_RPC_GETBLOCKHASH::request& req,
                                      ...
                                      COMMAND_RPC_GETBLOCKTEMPLATE::response& res)
  {
    ...
    if (0 < req.reserve_size)
    {
      ...
      res.reserved_offset +=
          sizeof(tx_pub_key) + 3;  // 3 bytes: tag for TX_EXTRA_TAG_PUBKEY(1 byte), tag for
  ...
  bool RpcServer::on_submitblock(const COMMAND_RPC_SUBMITBLOCK::request& req,
  ...
  void RpcServer::fill_block_header_response(const Block& blk, bool orphan_status, uint64_t height,
                                             const Hash& hash, block_header_response& responce)
  {
