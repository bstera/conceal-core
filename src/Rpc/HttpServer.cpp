...
namespace CryptoNote
{
  ...
  void HttpServer::start(const std::string& address, uint16_t port, const std::string& user,
  ...
  void HttpServer::acceptLoop()
  {
    try
    {
      ...
      for (;;)
      {
        ...
        {
          logger(WARNING) << "Authorization required " << addr.first.toDottedDecimal() << ":"
                          << addr.second;
      ...
      logger(DEBUGGING) << "Closing connection from " << addr.first.toDottedDecimal() << ":"
                        << addr.second << " total=" << m_connections.size();
    }
    catch (System::InterruptedException&)
    {
    }
  ...
  size_t HttpServer::get_connections_count() const { return m_connections.size(); }
