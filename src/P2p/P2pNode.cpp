...
namespace CryptoNote
{
  ...
  void P2pNode::acceptLoop()
  {
    while (!m_stopRequested)
    {
      try
      {
        ...
        auto ctx =
            new P2pContext(m_dispatcher, std::move(connection), true, getRemoteAddress(connection),
  ...
  {
    size_t peerIndex;
    PeerIndexGenerator idxGen(
        std::min<uint64_t>(peerlist.count() - 1, m_cfg.getPeerListConnectRange()));

    for (size_t tryCount = 0;
         idxGen.generate(peerIndex) && tryCount < m_cfg.getPeerListGetTryCount(); ++tryCount)
    {
      PeerlistEntry peer;
    ...
    try
    {
               ...
               CryptoNote::P2P_UPGRADE_WINDOW)
      {
        logger(WARNING) << *connection
                        << "COMMAND_HANDSHAKE Warning, your software may be out of date. Please "
  ...
  namespace
  {
    std::list<PeerlistEntry> fixTimeDelta(const std::list<PeerlistEntry>& peerlist,
                                          time_t remoteTime)
    {
      // fix time delta
  ...
  void P2pNode::handleNodeData(const basic_node_data& node, P2pContext& context)
  {
    ...
    {
      std::ostringstream msg;
      msg << context << "COMMAND_HANDSHAKE Failed, peer is wrong version! ("
