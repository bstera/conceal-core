...
namespace CryptoNote
{
  ...
  struct P2pConnectionContext : public CryptoNoteConnectionContext
  {
   public:
    ...
    P2pConnectionContext(System::Dispatcher& dispatcher, Logging::ILogger& log,
  ...
  class NodeServer : public IP2pEndpoint
  {
   ...
   private:
    ...
    int handleCommand(const LevinProtocol::Command& cmd, BinaryArray& buff_out,
                      P2pConnectionContext& context, bool& handled);

    //----------------- commands handlers ----------------------------------------------
    int handle_handshake(int command, COMMAND_HANDSHAKE::request& arg,
                         COMMAND_HANDSHAKE::response& rsp, P2pConnectionContext& context);
    int handle_timed_sync(int command, COMMAND_TIMED_SYNC::request& arg,
                          COMMAND_TIMED_SYNC::response& rsp, P2pConnectionContext& context);
    ...
    int handle_get_network_state(int command, COMMAND_REQUEST_NETWORK_STATE::request& arg,
                                 ...
                                 P2pConnectionContext& context);
    int handle_get_peer_id(int command, COMMAND_REQUEST_PEER_ID::request& arg,
    ...
    bool handshake(CryptoNote::LevinProtocol& proto, P2pConnectionContext& context,
    ...
    bool fix_time_delta(std::list<PeerlistEntry>& local_peerlist, time_t local_time,
                        int64_t& delta);
    ...
    typedef std::unordered_map<boost::uuids::uuid, P2pConnectionContext,
    ...
    void acceptLoop();
    void connectionHandler(const boost::uuids::uuid& connectionId,
