...
namespace CryptoNote
{
  ...
  class NodeRequest
  {
   public:
    ...
    void performAsync(BlockchainExplorer::AsyncContextCounter& asyncContextCounter,
                      const INode::Callback& callback)
    {
      asyncContextCounter.addAsyncContext();
      requestFunc(std::bind(&NodeRequest::asyncCompleteionCallback, callback,
                            std::ref(asyncContextCounter), std::placeholders::_1));
    }
   ...
   private:
    ...
    static void asyncCompleteionCallback(
        const INode::Callback& callback,
        BlockchainExplorer::AsyncContextCounter& asyncContextCounter, std::error_code ec)
    {
  ...
  bool BlockchainExplorer::PoolUpdateGuard::beginUpdate()
  {
    ...
    for (;;)
    {
      switch (state)
      {
        ...
        case State::UPDATE_REQUIRED: return false;
  ...
  class ScopeExitHandler
  {
   public:
    ScopeExitHandler(std::function<void()>&& handler)
        : m_handler(std::move(handler)), m_cancelled(false)
    {
    }
                                     ...
                                     std::vector<std::vector<BlockDetails>>& blocks)
  {
    ...
    NodeRequest request(std::bind(
        ...
        std::ref(node), std::cref(blockHeights), std::ref(blocks), std::placeholders::_1));
                                     ...
                                     std::vector<BlockDetails>& blocks)
  {
    ...
    NodeRequest request(std::bind(
        static_cast<void (INode::*)(const std::vector<Hash>&, std::vector<BlockDetails>&,
                                    const INode::Callback&)>(&INode::getBlocks),
        std::ref(node), std::cref(reinterpret_cast<const std::vector<Hash>&>(blockHashes)),
  ...
  bool BlockchainExplorer::getBlocks(uint64_t timestampBegin, uint64_t timestampEnd,
                                     ...
                                     uint32_t& blocksNumberWithinTimestamps)
  {
    ...
    NodeRequest request(std::bind(
        static_cast<void (INode::*)(uint64_t, uint64_t, uint32_t, std::vector<BlockDetails>&,
                                    uint32_t&, const INode::Callback&)>(&INode::getBlocks),
        std::ref(node), timestampBegin, timestampEnd, blocksNumberLimit, std::ref(blocks),
        std::ref(blocksNumberWithinTimestamps), std::placeholders::_1));
  ...
  bool BlockchainExplorer::getBlockchainTop(BlockDetails& topBlock)
  {
    ...
    {
      logger(ERROR) << "Can't get blockchain top: all blocks on height " << lastHeight
                    << " are orphaned.";
      throw std::system_error(
          make_error_code(CryptoNote::error::BlockchainExplorerErrorCodes::INTERNAL_ERROR));
    }
                                           ...
                                           std::vector<TransactionDetails>& transactions)
  {
    ...
    NodeRequest request(std::bind(
        static_cast<void (INode::*)(const std::vector<Hash>&, std::vector<TransactionDetails>&,
                                    const INode::Callback&)>(&INode::getTransactions),
        std::ref(node), std::cref(reinterpret_cast<const std::vector<Hash>&>(transactionHashes)),
                                               ...
                                               uint64_t& transactionsNumberWithinTimestamps)
  {
    ...
    NodeRequest request(std::bind(&INode::getPoolTransactions, std::ref(node), timestampBegin,
  ...
  bool BlockchainExplorer::getPoolState(const std::vector<Hash>& knownPoolTransactionHashes,
                                        Hash knownBlockchainTopHash, bool& isBlockchainActual,
                                        ...
                                        std::vector<Hash>& removedTransactions)
  {
    ...
    NodeRequest request([&](const INode::Callback& callback) {
      ...
      node.getPoolSymmetricDifference(
          std::move(hashes), reinterpret_cast<Hash&>(knownBlockchainTopHash), isBlockchainActual,
          rawNewTransactions, removedTransactions, callback);
    });
  ...
  bool BlockchainExplorer::isSynchronized()
  {
    ...
    NodeRequest request(std::bind(&INode::isSynchronized, std::ref(node), std::ref(syncStatus),
                                  std::placeholders::_1));
  ...
  void BlockchainExplorer::poolChanged()
  {
    ...
    NodeRequest request([this, rawNewTransactionsPtr, removedTransactionsPtr,
      ...
      node.getPoolSymmetricDifference(
          std::move(hashes), reinterpret_cast<Hash&>(knownBlockchainTop.hash),
    ...
    request.performAsync(asyncContextCounter, [this, rawNewTransactionsPtr, removedTransactionsPtr,
      ...
      std::shared_ptr<std::vector<Hash>> newTransactionsHashesPtr =
          std::make_shared<std::vector<Hash>>();
      ...
      std::shared_ptr<std::vector<std::pair<Hash, TransactionRemoveReason>>>
      ...
      NodeRequest request(std::bind(
          static_cast<void (INode::*)(const std::vector<Hash>&, std::vector<TransactionDetails>&,
                                      const INode::Callback&)>(&INode::getTransactions),
          std::ref(node), std::cref(*newTransactionsHashesPtr), std::ref(*newTransactionsPtr),
          std::placeholders::_1));
  ...
  void BlockchainExplorer::blockchainSynchronized(uint32_t topHeight)
  {
    ...
    std::shared_ptr<std::vector<uint32_t>> blockHeightsPtr =
    ...
    request.performAsync(asyncContextCounter, [this, blockHeightsPtr, blocksPtr,
                                               topHeight](std::error_code ec) {
      if (ec)
      {
        logger(ERROR)
      ...
      if (!gotMainchainBlock)
      {
        logger(ERROR) << "Can't send blockchainSynchronized notification because can't get "
                         "blockchain top: all blocks on height "
  ...
  void BlockchainExplorer::localBlockchainUpdated(uint32_t height)
  {
    ...
    request.performAsync(asyncContextCounter, [this, blockHeightsPtr,
                                               blocksPtr](std::error_code ec) {
      if (ec)
      {
        logger(ERROR)
      ...
      if (!gotTopMainchainBlock)
      {
        logger(ERROR) << "Can't send localBlockchainUpdated notification because can't get "
                         "blockchain top: all blocks on height "
                      << topHeight << " are orphaned.";
        return;
      }
