...
namespace CryptoNote
{
  BlockchainExplorerDataBuilder::BlockchainExplorerDataBuilder(
      CryptoNote::ICore& core, CryptoNote::ICryptoNoteProtocolQuery& protocol)
                                                  ...
                                                  TransactionExtraDetails& extraDetails)
  {
    ...
    for (const TransactionExtraField& field : txExtraFields)
    {
      ...
      {
        extraDetails.nonce.push_back(
            Common::toHex(boost::get<TransactionExtraNonce>(field).nonce.data(),
                                                       ...
                                                       BlockDetails& blockDetails)
  {
    ...
    if (!core.getGeneratedTransactionsNumber(blockDetails.height,
    ...
    if (!core.getBlockReward(blockDetails.sizeMedian, 0, prevBlockGeneratedCoins, 0,
                             blockDetails.height, maxReward, emissionChange))
    {
      return false;
    }
    if (!core.getBlockReward(blockDetails.sizeMedian, blockDetails.transactionsCumulativeSize,
                             prevBlockGeneratedCoins, 0, blockDetails.height, currentReward,
                             emissionChange))
    {
      return false;
    }
                                                             ...
                                                             uint64_t timestamp)
  {
    ...
    transactionDetails.totalInputsAmount =
        core.currency().getTransactionAllInputsAmount(transaction, transactionDetails.blockHeight);
    ...
    else
    {
      transactionDetails.fee =
          inputsAmount < transactionDetails.totalOutputsAmount
              ? CryptoNote::parameters::MINIMUM_FEE
              : core.currency().getTransactionFee(transaction, transactionDetails.blockHeight);
      uint64_t mixin;
    ...
    for (const outputWithIndex& txOutput : range)
    {
      ...
      else if (txOutput.get<0>().target.type() == typeid(MultisignatureOutput))
      {
        ...
        MultisignatureOutput txOutMultisig =
            boost::get<MultisignatureOutput>(txOutput.get<0>().target);
