...
namespace System
{
  ...
  size_t TcpConnection::read(uint8_t* data, size_t size)
  {
    ...
    dispatcher->getCurrentContext()->interruptProcedure = [&]() {
      ...
      if (!context->interrupted)
      {
        if (CancelIoEx(reinterpret_cast<HANDLE>(connection), context) != TRUE)
        {
          ...
          {
            throw std::runtime_error("TcpConnection::stop, CancelIoEx failed, " +
                                     lastErrorMessage());
          }
    ...
    if (WSAGetOverlappedResult(connection, &context, &transferred, FALSE, &flags) != TRUE)
    {
      ...
      {
        throw std::runtime_error("TcpConnection::read, WSAGetOverlappedResult failed, " +
  ...
  size_t TcpConnection::write(const uint8_t* data, size_t size)
  {
    ...
    if (size == 0)
    {
      ...
      {
        throw std::runtime_error("TcpConnection::write, shutdown failed, " +
                                 errorMessage(WSAGetLastError()));
      }
    ...
    if (WSASend(connection, &buf, 1, NULL, 0, &context, NULL) != 0)
    {
      ...
      if (lastError != WSA_IO_PENDING)
      {
        throw std::runtime_error("TcpConnection::write, WSASend failed, " +
                                 errorMessage(lastError));
      }
    }
    ...
    if (WSAGetOverlappedResult(connection, &context, &transferred, FALSE, &flags) != TRUE)
    {
      ...
      if (lastError != ERROR_OPERATION_ABORTED)
      {
        throw std::runtime_error("TcpConnection::write, WSAGetOverlappedResult failed, " +
                                 errorMessage(lastError));
      }
  ...
  std::pair<Ipv4Address, uint16_t> TcpConnection::getPeerAddressAndPort() const
  {
    ...
    return std::make_pair(Ipv4Address(htonl(address.sin_addr.S_un.S_addr)),
