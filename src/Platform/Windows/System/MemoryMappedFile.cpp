...
namespace System
{
                                ...
                                std::error_code& ec)
  {
    ...
    m_fileHandle = ::CreateFile(
        path.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_DELETE | FILE_SHARE_READ, NULL,
    ...
    DWORD filePointer =
        ::SetFilePointer(m_fileHandle, static_cast<LONG>(size & UINT64_C(0xffffffff)),
    ...
    m_data =
        reinterpret_cast<uint8_t*>(::MapViewOfFile(m_mappingHandle, FILE_MAP_ALL_ACCESS, 0, 0, 0));
    if (m_data == NULL)
    {
      return;
    }
  ...
  void MemoryMappedFile::open(const std::string& path, std::error_code& ec)
  {
    ...
    m_fileHandle = ::CreateFile(path.c_str(), GENERIC_READ | GENERIC_WRITE,
