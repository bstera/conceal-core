...
namespace System
{
  ...
  TcpListener& TcpListener::operator=(TcpListener&& other)
  {
    if (dispatcher != nullptr)
    {
      ...
      {
        throw std::runtime_error("TcpListener::operator=, closesocket failed, " +
  ...
  TcpConnection TcpListener::accept()
  {
    ...
    else
    {
      ...
      if (acceptEx(listener, connection, addresses, 0, sizeof(sockaddr_in) + 16,
                   sizeof(sockaddr_in) + 16, &received, &context2) == TRUE)
      {
          ...
          dispatcher->getCurrentContext()->interruptProcedure = [&]() {
            ...
            if (!context2->interrupted)
            {
              if (CancelIoEx(reinterpret_cast<HANDLE>(listener), context2) != TRUE)
              {
                ...
                {
                  throw std::runtime_error("TcpListener::stop, CancelIoEx failed, " +
                                           lastErrorMessage());
                }
