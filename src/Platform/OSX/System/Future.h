...
namespace System
{
  namespace Detail
  {
    ...
    // Simplest possible future implementation. The reason why this class even exist is because
    // currenty std future has a memory corrupting bug on OSX. Spawn a new thread, execute procedure
    // in it, get result, and wait thread to shut down. Actualy, this is what libstdc++'s
    // std::future is doing.
    template<class T>
    class Future
    {
