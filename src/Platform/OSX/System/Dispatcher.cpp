...
namespace System
{
    ...
    class MutextGuard
    {
     public:
      MutextGuard(pthread_mutex_t& _mutex) : mutex(_mutex)
      {
        ...
        if (ret != 0)
        {
          throw std::runtime_error("MutextGuard::MutextGuard, pthread_mutex_lock failed, " +
                                   errorMessage(ret));
        }
      }
    ...
    for (;;)
    {
      ...
      if (count > 0)
      {
        for (int i = 0; i < count; ++i)
        {
          ...
          {
            EV_SET(&updates[updatesCounter++], 0, EVFILT_USER, EV_ADD | EV_DISABLE, NOTE_FFNOP, 0,
                   NULL);
  ...
  NativeContext& Dispatcher::getReusableContext()
  {
    if (firstReusableContext == nullptr)
    {
      ...
      makecontext(static_cast<uctx*>(newlyCreatedContext),
