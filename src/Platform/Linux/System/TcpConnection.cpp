...
namespace System
{
      ...
      else
      {
        ...
        else
        {
          dispatcher->getCurrentContext()->interruptProcedure = [&]() {
            ...
            if (epoll_ctl(dispatcher->getEpoll(), EPOLL_CTL_MOD, connection, &connectionEvent) ==
                -1)
            {
              throw std::runtime_error("TcpConnection::stop, epoll_ctl failed, " +
                                       lastErrorMessage());
            }
      ...
      : dispatcher(&dispatcher), connection(socket)
  {
    ...
    {
      throw std::runtime_error("TcpConnection::TcpConnection, epoll_ctl failed, " +
                               lastErrorMessage());
    }
  }
