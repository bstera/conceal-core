...
namespace System
{
  ...
  Dispatcher::~Dispatcher()
  {
    for (NativeContext* context = contextGroup.firstContext; context != nullptr;
  ...
  void Dispatcher::yield()
  {
    for (;;)
    {
      ...
      if (count > 0)
      {
        for (int i = 0; i < count; ++i)
        {
          ...
          if (((events[i].events & (EPOLLIN | EPOLLOUT)) != 0) &&
              contextPair->readContext == nullptr && contextPair->writeContext == nullptr)
          {
            uint64_t buf;
            auto transferred = read(remoteSpawnEvent, &buf, sizeof buf);
            if (transferred == -1)
      ...
      else
      {
        if (errno != EINTR)
        {
          throw std::runtime_error("Dispatcher::dispatch, epoll_wait failed, " +
                                   lastErrorMessage());
        }
      }
    }
  ...
  NativeContext& Dispatcher::getReusableContext()
  {
    if (firstReusableContext == nullptr)
    {
      ...
      {  // makecontext precondition
        throw std::runtime_error("Dispatcher::getReusableContext, getcontext failed, " +
                                 lastErrorMessage());
      }
      ...
      {
        throw std::runtime_error("Dispatcher::getReusableContext, swapcontext failed, " +
                                 lastErrorMessage());
      }
  ...
  void Dispatcher::contextProcedure(void* ucontext)
  {
    ...
    {
      throw std::runtime_error("Dispatcher::contextProcedure, swapcontext failed, " +
                               lastErrorMessage());
    }
