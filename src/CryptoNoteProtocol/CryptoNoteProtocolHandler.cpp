...
namespace CryptoNote
{
  namespace
  {
    ...
    bool post_notify(IP2pEndpoint &p2p, typename t_parametr::request &arg,
  ...
  CryptoNoteProtocolHandler::CryptoNoteProtocolHandler(const Currency &currency,
                                                       System::Dispatcher &dispatcher, ICore &rcore,
                                                            ...
                                                            bool is_inital)
  {
    ...
    else
    {
      ...
      logger(diff >= 0 ? (is_inital ? Logging::INFO : DEBUGGING) : Logging::TRACE)
          << context << "Unknown top block: " << get_current_blockchain_height() << " -> "
          << hshd.current_height << std::endl

          << "Synchronization started";

      logger(DEBUGGING) << "Remote top block height: " << hshd.current_height
                        << ", id: " << hshd.top_id;
      // let the socket to send response to handshake, but request callback, to let send request
      // data after response
...
#define HANDLE_NOTIFY(CMD, Handler)                                                    \
  case CMD::ID:                                                                        \
  {                                                                                    \
    ret = notifyAdaptor<CMD>(in, ctx,                                                  \
                             std::bind(Handler, this, std::placeholders::_1,           \
                                       std::placeholders::_2, std::placeholders::_3)); \
    break;                                                                             \
  }

  int CryptoNoteProtocolHandler::handleCommand(bool is_notify, int command, const BinaryArray &in,
                                               BinaryArray &out, CryptoNoteConnectionContext &ctx,
                                               bool &handled)
  {
    int ret = 0;
    handled = true;
    ...
    switch (command)
    {
      ...
      HANDLE_NOTIFY(NOTIFY_NEW_TRANSACTIONS,
                    &CryptoNoteProtocolHandler::handle_notify_new_transactions)
      ...
      HANDLE_NOTIFY(NOTIFY_RESPONSE_CHAIN_ENTRY,
                    &CryptoNoteProtocolHandler::handle_response_chain_entry)
...
#undef HANDLE_NOTIFY
                                                         ...
                                                         CryptoNoteConnectionContext &context)
  {
    ...
    for (auto tx_blob_it = arg.b.txs.begin(); tx_blob_it != arg.b.txs.end(); tx_blob_it++)
    {
      ...
      // transactionBinary.size()); logger(DEBUGGING) << "transaction " << transactionHash << " came
      // in NOTIFY_NEW_BLOCK";
  ...
  int CryptoNoteProtocolHandler::handle_notify_new_transactions(
      int command, NOTIFY_NEW_TRANSACTIONS::request &arg, CryptoNoteConnectionContext &context)
  {
    ...
    if (context.m_pending_lite_block)
    {
      logger(Logging::TRACE) << context
                             << " Pending lite block detected, handling request as missing lite "
    ...
    else
    {
      for (auto tx_blob_it = arg.txs.begin(); tx_blob_it != arg.txs.end();)
      {
        ...
        Crypto::Hash transactionHash =
            Crypto::cn_fast_hash(transactionBinary.data(), transactionBinary.size());
  ...
  int CryptoNoteProtocolHandler::handle_request_get_objects(
      int command, NOTIFY_REQUEST_GET_OBJECTS::request &arg, CryptoNoteConnectionContext &context)
  {
    ...
    if (!m_core.handle_get_objects(arg, rsp))
    {
      logger(Logging::ERROR)
          << context << "failed to handle request NOTIFY_REQUEST_GET_OBJECTS, dropping connection";
  ...
  int CryptoNoteProtocolHandler::handle_response_get_objects(
      int command, NOTIFY_RESPONSE_GET_OBJECTS::request &arg, CryptoNoteConnectionContext &context)
  {
    ...
    for (const block_complete_entry &block_entry : arg.blocks)
    {
      ...
      {
        logger(Logging::ERROR) << context << "sent wrong block: too big size " << block_blob.size()
                               << ", dropping connection";
        context.m_state = CryptoNoteConnectionContext::state_shutdown;
        return 1;
      }
      ...
      // to avoid concurrency in core between connections, suspend connections which delivered block
      // later then first one
    ...
    Crypto::Hash top;
    {
      ...
      for (const auto &h : block_hashes)
      {
        if (top == h)
        {
          logger(DEBUGGING) << "Found current top block in synced blocks, dismissing " << dismiss
                            << "/" << arg.blocks.size() << " blocks";
          while (dismiss--)
          {
                                                ...
                                                const std::vector<parsed_block_entry> &blocks)
  {
    for (const parsed_block_entry &block_entry : blocks)
    {
      ...
      for (size_t i = 0; i < block_entry.txs.size(); ++i)
      {
        ...
        {
          logger(DEBUGGING) << context
                            << "transaction mismatch on NOTIFY_RESPONSE_GET_OBJECTS, \r\ntx_id = "
  ...
  bool CryptoNoteProtocolHandler::on_idle() { return m_core.on_idle(); }
                                                          ...
                                                          bool check_having_blocks)
  {
    if (context.m_needed_objects.size())
    {
      ...
      logger(Logging::TRACE) << context << "-->>NOTIFY_REQUEST_GET_OBJECTS: blocks.size()="
                             << req.blocks.size() << ", txs.size()=" << req.txs.size();
  ...
  bool CryptoNoteProtocolHandler::on_connection_synchronized()
  {
    ...
    if (m_synchronized.compare_exchange_strong(val_expected, true))
    {
      logger(Logging::INFO)
          << ENDL
          << "********************************************************************************"
          << ENDL << "You are now synchronized with the Conceal network." << ENDL
  ...
  int CryptoNoteProtocolHandler::handle_response_chain_entry(
      int command, NOTIFY_RESPONSE_CHAIN_ENTRY::request &arg, CryptoNoteConnectionContext &context)
  {
    ...
    context.m_last_response_height =
        arg.start_height + static_cast<uint32_t>(arg.m_block_ids.size()) - 1;
  ...
  int CryptoNoteProtocolHandler::handle_notify_missing_txs(int command,
                                                           ...
                                                           CryptoNoteConnectionContext &context)
  {
    ...
    if (!missedHashes.empty())
    {
      logger(Logging::DEBUGGING)
          << "Failed to Handle NOTIFY_MISSING_TXS, Unable to retrieve requested "
  ...
  int CryptoNoteProtocolHandler::handle_request_tx_pool(int command,
  ...
  void CryptoNoteProtocolHandler::relay_block(NOTIFY_NEW_BLOCK::request &arg)
  {
    ...
    m_p2p->for_each_connection([this, &liteBlockConnections, &normalBlockConnections](
      ...
      {
        logger(Logging::DEBUGGING)
            << ctx << "Peer supports lite-blocks... adding peer to lite block list";
                                                       ...
                                                       const CryptoNoteConnectionContext &context)
  {
    bool updated = false;
    {
      ...
      {
        // the client switched to alternative chain and had maximum observed height. need to
      ...
      const CryptoNoteConnectionContext &context)
  {
    ...
    m_p2p->for_each_connection(
        [&peerHeight, &context](const CryptoNoteConnectionContext &ctx, PeerIdType peerId) {
                                                 ...
                                                 std::vector<BinaryArray> missingTxs)
  {
    ...
    if (need_txs.empty())
    {
      ...
      for (auto transactionBinary : have_txs)
      {
        ...
        if (tvc.m_verification_failed)
        {
          logger(Logging::INFO) << context
                                << "Lite block verification failed: transaction verification "
      ...
      {
        logger(Logging::DEBUGGING)
            << context << "Lite block verification failed, dropping connection";
        m_p2p->drop_connection(context, true);
        return 1;
      }
      if (bvc.m_added_to_main_chain)
    ...
    else
    {
      ...
      else
      {
        ...
        context.m_pending_lite_block =
            PendingLiteBlock{ arg, { req.missing_txs.begin(), req.missing_txs.end() } };
