...
namespace CryptoNote
{
  ...
  void Miner::runWorkers(BlockMiningParameters blockMiningParameters, size_t threadCount)
  {
    ...
    m_logger(Logging::INFO) << "Starting mining for difficulty "
    ...
    try
    {
      ...
      for (size_t i = 0; i < threadCount; ++i)
      {
        m_workers.emplace_back(
            std::unique_ptr<System::RemoteContext<void>>(new System::RemoteContext<void>(
                m_dispatcher,
                std::bind(&Miner::workerFunc, this, blockMiningParameters.blockTemplate,
                          blockMiningParameters.difficulty, threadCount))));
  ...
  bool Miner::setStateBlockFound()
  {
    ...
    for (;;)
    {
      switch (state)
      {
        case MiningState::BLOCK_FOUND: return false;
        ...
        case MiningState::MINING_STOPPED: return false;
