...
namespace CryptoNote
{
  namespace
  {
    ...
    void parseDaemonAddress(const std::string& daemonAddress, std::string& daemonHost,
                            uint16_t& daemonPort)
    {
  ...
  {
    cmdOptions.add_options()("help,h", "produce this help message and exit")(
        ...
        "Daemon's RPC port")("daemon-address", po::value<std::string>(),
        ...
        "Mining threads count. Must not be greater than you concurrency level. Default value is "
        "your hardware concurrency level")(
        ...
        "limit", po::value<size_t>()->default_value(0),
        ...
        "Timestamp step for each subsequent block. May be set only if --first-block-timestamp has "
        "been set."
  ...
  void MiningConfig::parse(int argc, char** argv)
  {
    ...
    if (!options["daemon-address"].empty())
    {
      if (!options["daemon-host"].defaulted() || !options["daemon-rpc-port"].defaulted())
      {
        throw std::runtime_error(
            "Either --daemon-host or --daemon-rpc-port is already specified. You must not specify "
            "--daemon-address");
      }
    ...
    {
      throw std::runtime_error(
          "If you specify --block-timestamp-interval you must specify --first-block-timestamp "
          "either");
    }
  ...
  void MiningConfig::printHelp() { std::cout << cmdOptions << std::endl; }
