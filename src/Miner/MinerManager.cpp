...
namespace Miner
{
  ...
  MinerManager::MinerManager(System::Dispatcher& dispatcher, const CryptoNote::MiningConfig& config,
      ...
      : m_dispatcher(dispatcher),
        ...
        m_blockchainMonitor(dispatcher, m_config.daemonHost, m_config.daemonPort,
                            m_config.scanPeriod, logger),
          ...
          if (submitBlock(m_minedBlock, m_config.daemonHost, m_config.daemonPort))
          {
            ...
            if (m_config.blocksLimit != 0 && ++blocksMined == m_config.blocksLimit)
            {
              m_logger(Logging::INFO)
                  << "Miner mined requested " << m_config.blocksLimit << " blocks. Quitting";
              return;
            }
          }

          BlockMiningParameters params = requestMiningParameters(
              m_dispatcher, m_config.daemonHost, m_config.daemonPort, m_config.miningAddress);
          adjustBlockTemplate(params.blockTemplate);
  ...
  bool MinerManager::submitBlock(const Block& minedBlock, const std::string& daemonHost,
                                 uint16_t daemonPort)
  {
    try
    {
    ...
    {
      m_logger(Logging::WARNING) << "Couldn't submit block: "
                                 << Common::podToHex(get_block_hash(minedBlock))
                                 << ", reason: " << e.what();
      return false;
    }
