...
namespace
{
  ...
  void checkOutputKey(const KeyDerivation& derivation, const PublicKey& key, size_t keyIndex,
                      size_t outputIndex, const std::unordered_set<PublicKey>& spendKeys,
...
namespace CryptoNote
{
  TransfersConsumer::TransfersConsumer(const CryptoNote::Currency& currency, INode& node,
                                       Logging::ILogger& logger, const SecretKey& viewSecret)
      : m_node(node),
  ...
  ITransfersSubscription& TransfersConsumer::addSubscription(
  ...
  bool TransfersConsumer::onNewBlocks(const CompleteBlock* blocks, uint32_t startHeight,
                                      uint32_t count)
  {
    assert(blocks);
    assert(count > 0);
    ...
    {
      forEachSubscription(
          [&](TransfersSubscription& sub) { sub.onError(processingError, startHeight); });
      ...
      const std::vector<Hash>& deletedTransactions)
  {
    ...
    for (auto& deletedTxHash : deletedTransactions)
    {
      ...
      m_observerManager.notify(&IBlockchainConsumerObserver::onTransactionDeleteBegin, this,
                               deletedTxHash);
      for (auto& sub : m_subscriptions)
      {
      ...
      m_observerManager.notify(&IBlockchainConsumerObserver::onTransactionDeleteEnd, this,
                               deletedTxHash);
    }
  ...
  std::error_code TransfersConsumer::addUnconfirmedTransaction(
  ...
  void TransfersConsumer::addPublicKeysSeen(const Crypto::Hash& transactionHash,
                                  ...
                                  std::vector<TransactionOutputInformationIn>& transfers)
  {
    ...
    for (auto idx : outputs)
    {
      ...
      if (outType == TransactionTypes::OutputType::Key)
      {
        ...
        std::unordered_set<Crypto::Hash>::iterator it =
            transactions_hash_seen.find(tx.getTransactionHash());
      ...
      else if (outType == TransactionTypes::OutputType::Multisignature)
      {
        ...
        for (const auto& key : out.keys)
        {
          ...
          if (it == transactions_hash_seen.end())
          {
            ...
            {
              //  m_logger(ERROR, BRIGHT_RED) << "Failed to process transaction " <<
              //  Common::podToHex(txHash) << ": the same multisignature output key is present more
              //  than once";
              return std::error_code();
            }
                                                       ...
                                                       PreprocessInfo& info)
  {
    ...
    {
      m_logger(ERROR, BRIGHT_RED) << "Failed to process transaction: " << e.what()
                                  << ", transaction hash "
    ...
    for (const auto& kv : outputs)
    {
      ...
      if (it != m_subscriptions.end())
      {
        ...
        try
        {
          errorCode = createTransfers(it->second->getKeys(), blockInfo, tx, kv.second,
                                      info.globalIdxs, transfers);
          if (errorCode)
          {
            return errorCode;
          }
        ...
        catch (const std::exception& e)
        {
          m_logger(ERROR, BRIGHT_RED)
              << "Failed to process transaction: " << e.what() << ", transaction hash "
              << Common::podToHex(tx.getTransactionHash());
          return std::error_code();
        }
      }
  ...
  void TransfersConsumer::processTransaction(const TransactionBlockInfo& blockInfo,
                                             const ITransactionReader& tx,
                                             const PreprocessInfo& info)
  {
    ...
    for (auto& kv : m_subscriptions)
    {
      ...
      processOutputs(blockInfo, *kv.second, tx, subscriptionOutputs, info.globalIdxs,
                     containerContainsTx, containerUpdated);
    ...
    {
      m_observerManager.notify(&IBlockchainConsumerObserver::onTransactionUpdated, this,
                               tx.getTransactionHash(), transactionContainers);
    }
  }

  void TransfersConsumer::processOutputs(
      const TransactionBlockInfo& blockInfo, TransfersSubscription& sub,
      ...
      const std::vector<uint32_t>& globalIdxs, bool& contains, bool& updated)
  {
    ...
    contains =
        sub.getContainer().getTransactionInformation(tx.getTransactionHash(), subscribtionTxInfo);
    updated = false;
