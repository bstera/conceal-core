...
namespace
{
  ...
  {
    std::vector<uint8_t> vec(reinterpret_cast<const uint8_t*>(s.data()),
...
namespace CryptoNote
{
  ...
  void BlockchainSynchronizer::addConsumer(IBlockchainConsumer* consumer)
  {
    ...
    m_consumers.insert(
        std::make_pair(consumer, std::make_shared<SynchronizationState>(m_genesisBlockHash)));
  }
  ...
  bool BlockchainSynchronizer::removeConsumer(IBlockchainConsumer* consumer)
  {
    ...
    if (!(checkIfStopped() && checkIfShouldStop()))
    {
      throw std::runtime_error(
          "Can't remove consumer, because BlockchainSynchronizer isn't stopped");
    }
      ...
      const Crypto::Hash& transactionHash)
  {
    ...
    {
      throw std::runtime_error(
          "Can't remove transaction, because BlockchainSynchronizer is stopped");
    }
  ...
  {
    std::unique_lock<std::mutex> lk(m_stateMutex);
    if (m_currentState == State::stopped && m_futureState == State::blockchainSync)
    {  // start(), immediately attach observer
      m_node.addObserver(this);
    }

    if (m_futureState == State::stopped && m_currentState != State::stopped)
    {  // stop(), immediately detach observer
  ...
  void BlockchainSynchronizer::start()
  {
    ...
    if (!setFutureStateIf(State::blockchainSync, [this] {
  ...
  void BlockchainSynchronizer::poolChanged() { setFutureState(State::poolSync); }
  //--------------------------- FSM END ------------------------------------
      ...
      std::unordered_set<Crypto::Hash>& poolIntersection) const
  {
    ...
    {
      const std::unordered_set<Crypto::Hash>& consumerKnownIds =
          itConsumers->first->getKnownPoolTxIds();

      poolUnion.insert(consumerKnownIds.begin(), consumerKnownIds.end());

      for (auto itIntersection = poolIntersection.begin();
           itIntersection != poolIntersection.end();)
      {
  ...
  void BlockchainSynchronizer::processBlocks(GetBlocksResponse& response)
  {
    ...
    for (auto& block : response.newBlocks)
    {
      ...
      if (block.hasBlock)
      {
        ...
        completeBlock.transactions.push_back(
            createTransactionPrefix(completeBlock.block->baseTransaction));
    ...
    uint32_t processedBlockCount =
        response.startHeight + static_cast<uint32_t>(response.newBlocks.size());
    if (!checkIfShouldStop())
    {
      ...
      switch (result)
      {
        ...
        case UpdateConsumersResult::addedNewBlocks:
          ...
          m_observerManager.notify(
              &IBlockchainSynchronizerObserver::synchronizationProgressUpdated, processedBlockCount,
      ...
      const BlockchainInterval& interval, const std::vector<CompleteBlock>& blocks)
  {
    ...
    for (auto& kv : m_consumers)
    {
      ...
      if (result.hasNewBlocks)
      {
        ...
        // update consumer
        if (kv.first->onNewBlocks(blocks.data() + startOffset, result.newBlockHeight,
                                  static_cast<uint32_t>(blocks.size()) - startOffset))
        {
          // update state if consumer succeeded
          kv.second->addBlocks(interval.blocks.data() + startOffset, result.newBlockHeight,
    ...
    return smthChanged ? UpdateConsumersResult::addedNewBlocks
                                                                         ...
                                                                         GetPoolResponse& response)
  {
    ...
    m_node.getPoolSymmetricDifference(std::move(request.knownTxIds),
                                      std::move(request.lastKnownBlock),
                                      response.isLastKnownBlockActual, response.newTxs,
                                      response.deletedTxIds, [&promise](std::error_code ec) {
      ...
      IBlockchainConsumer* consumer) const
  {
    ...
    {
      throw std::runtime_error(
          "Can't get consumer state, because BlockchainSynchronizer isn't stopped");
    }
