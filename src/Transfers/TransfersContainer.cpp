...
namespace CryptoNote
{
  ...
  namespace
  {
    ...
    class TransferIteratorList
    {
     public:
      ...
      {
        auto listIt = std::find_if(m_list.begin(), m_list.end(),
                                   [amount](const TIterator& it) { return it->amount == amount; });
      ...
      {
        auto listIt =
            std::min_element(m_list.begin(), m_list.end(), &TransferIteratorList::lessTIterator);
        return listIt == m_list.end() ? m_end : *listIt;
      }
    ...
    TransferIteratorList<TIterator> createTransferIteratorList(
        const std::pair<TIterator, TIterator>& itPair)
    {
      return TransferIteratorList<TIterator>(itPair.first, itPair.second);
    }

    TransferUnlockJob makeTransferUnlockJob(const TransactionOutputInformationEx& output,
                                            uint32_t transactionSpendableAge)
    {
      TransferUnlockJob job;

      uint32_t unlockTime =
          static_cast<uint32_t>(output.unlockTime == 0 ? output.blockHeight : output.unlockTime);

      if (output.type == TransactionTypes::OutputType::Multisignature && output.term != 0)
      {
        job.unlockHeight = std::max({ unlockTime, output.blockHeight + output.term,
                                      output.blockHeight + transactionSpendableAge + 1 });
      }
      else
      {
  ...
  SpentOutputDescriptor::SpentOutputDescriptor(
      const TransactionOutputInformationIn& transactionInfo)
  ...
  bool SpentOutputDescriptor::operator==(const SpentOutputDescriptor& other) const
  {
    ...
    {
      return other.m_type == m_type && other.m_amount == m_amount &&
  ...
  TransfersContainer::TransfersContainer(const Currency& currency, size_t transactionSpendableAge)
      : m_currentHeight(0), m_currency(currency), m_transactionSpendableAge(transactionSpendableAge)
  {
  }

  bool TransfersContainer::addTransaction(
      const TransactionBlockInfo& block, const ITransactionReader& tx,
   ...
   * \pre m_mutex is locked.
   */
  bool TransfersContainer::addTransactionInputs(const TransactionBlockInfo& block,
                                                const ITransactionReader& tx)
  {
    ...
    for (size_t i = 0; i < tx.getInputCount(); ++i)
    {
      ...
      else if (inputType == TransactionTypes::InputType::Multisignature)
      {
        ...
        auto& outputDescriptorIndex = m_availableTransfers.get<SpentOutputDescriptorIndex>();
        auto availableOutputIt =
            outputDescriptorIndex.find(SpentOutputDescriptor(input.amount, input.outputIndex));
                                                    ...
                                                    const std::vector<uint32_t>& globalIndices)
  {
    if (block.height == WALLET_LEGACY_UNCONFIRMED_TRANSACTION_HEIGHT)
    {
      throw std::invalid_argument(
          "Block height equals WALLET_LEGACY_UNCONFIRMED_TRANSACTION_HEIGHT");
    }
  ...
  void TransfersContainer::deleteTransactionTransfers(const Crypto::Hash& transactionHash)
  {
    ...
    for (auto it = spentTransfersRange.first; it != spentTransfersRange.second;)
    {
      ...
      const TransactionOutputInformationEx& unspendingTransfer =
  ...
  void TransfersContainer::copyToSpent(const TransactionBlockInfo& block,
                                       const ITransactionReader& tx, size_t inputIndex,
  ...
  void TransfersContainer::detach(uint32_t height, std::vector<Crypto::Hash>& deletedTransactions,
                                  std::vector<TransactionOutputInformation>& lockedTransfers)
  {
    // This method expects that WALLET_LEGACY_UNCONFIRMED_TRANSACTION_HEIGHT is a big positive
    // number
  ...
  void TransfersContainer::getOutputs(std::vector<TransactionOutputInformation>& transfers,
  ...
  bool TransfersContainer::getTransactionInformation(const Crypto::Hash& transactionHash,
  ...
  std::vector<TransactionOutputInformation> TransfersContainer::getTransactionOutputs(
      const Crypto::Hash& transactionHash, uint32_t flags) const
  {
    std::lock_guard<std::mutex> lk(m_mutex);
  ...
  bool TransfersContainer::getTransfer(const Crypto::Hash& transactionHash,
                                       ...
                                       TransferState& transferState) const
  {
    ...
    if (availableIt != availableIndex.end())
    {
      ...
      if (!isSpendTimeUnlocked(*availableIt) ||
          m_currentHeight < availableIt->blockHeight + m_transactionSpendableAge)
      {
  ...
  void TransfersContainer::save(std::ostream& os)
  {
    ...
    writeSequence<TransactionInformation>(m_transactions.begin(), m_transactions.end(),
                                          "transactions", s);
    ...
    writeSequence<SpentTransactionOutput>(m_spentTransfers.begin(), m_spentTransfers.end(),
                                          "spentTransfers", s);
    writeSequence<TransferUnlockJob>(m_transfersUnlockJobs.begin(), m_transfersUnlockJobs.end(),
                                     "transfersUnlockJobs", s);
  }
  ...
  void TransfersContainer::load(std::istream& in)
  {
    ...
    readSequence<TransactionInformation>(std::inserter(transactions, transactions.end()),
                                         "transactions", s);
    ...
    readSequence<SpentTransactionOutput>(std::inserter(spentTransfers, spentTransfers.end()),
                                         "spentTransfers", s);

    if (version != 0)
    {
      readSequence<TransferUnlockJob>(std::inserter(transfersUnlockJobs, transfersUnlockJobs.end()),
                                      "transfersUnlockJobs", s);
    }
    else
    {
  ...
  void TransfersContainer::rebuildTransfersUnlockJobs(
      TransfersUnlockMultiIndex& transfersUnlockJobs,
      const AvailableTransfersMultiIndex& availableTransfers,
      const SpentTransfersMultiIndex& spentTransfers)
  {
    for (auto it = availableTransfers.begin(); it != availableTransfers.end(); ++it)
    {
      TransferUnlockJob job =
          makeTransferUnlockJob(*it, static_cast<uint32_t>(m_transactionSpendableAge));
  ...
  bool TransfersContainer::isSpendTimeUnlocked(const TransactionOutputInformationEx& info) const
  {
    ...
    if (info.unlockTime < m_currency.maxBlockHeight())
    {
      ...
      isOuputUnlocked =
          m_currentHeight + m_currency.lockedTxAllowedDeltaBlocks() >= info.unlockTime;
    }
    else
    {
      // interpret as time
    ...
    if (isOuputUnlocked && info.type == TransactionTypes::OutputType::Multisignature &&
        info.term != 0)
    {
  ...
  bool TransfersContainer::isIncluded(const TransactionOutputInformationEx& info,
                                      uint32_t flags) const
  {
    uint32_t state;
    if (info.blockHeight == WALLET_LEGACY_UNCONFIRMED_TRANSACTION_HEIGHT ||
        !isSpendTimeUnlocked(info))
    {
  ...
  bool TransfersContainer::isIncluded(const TransactionOutputInformationEx& output, uint32_t state,
                                      uint32_t flags)
  {
    return
        // filter by type
        (((flags & IncludeTypeKey) != 0 && output.type == TransactionTypes::OutputType::Key) ||
         ((flags & IncludeTypeMultisignature) != 0 &&
          output.type == TransactionTypes::OutputType::Multisignature && output.term == 0) ||
         ((flags & IncludeTypeDeposit) != 0 &&
          output.type == TransactionTypes::OutputType::Multisignature && output.term > 0)) &&
        // filter by state
        ((flags & state) != 0);
  }
  ...
  {
    TransferUnlockJob job =
        makeTransferUnlockJob(output, static_cast<uint32_t>(m_transactionSpendableAge));
   ...
   *  \pre m_mutex is locked
   */
  std::vector<TransactionOutputInformation> TransfersContainer::getUnlockingTransfers(
      uint32_t prevHeight, uint32_t currentHeight)
  {
   ...
   *  \pre m_mutex is locked
   */
  void TransfersContainer::getLockingTransfers(
      uint32_t prevHeight, uint32_t currentHeight,
   ...
   */
  TransactionOutputInformation TransfersContainer::getAvailableOutput(
      const TransactionOutputKey& transactionOutputKey) const
  {
    auto& availableIndex = m_availableTransfers.get<TransactionOutputKeyIndex>();
