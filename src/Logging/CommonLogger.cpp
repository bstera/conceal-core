...
namespace Logging
{
                              ...
                              boost::posix_time::ptime time)
    {
      ...
      for (const char* p = pattern.c_str(); p && *p != 0; ++p)
      {
        if (*p == '%')
        {
          ...
          {
            case 0: break;
            case 'C': s << category; break;
            case 'D': s << time.date(); break;
            case 'T': s << time.time_of_day(); break;
            case 'L': s << ILogger::LEVEL_NAMES[level]; break;
            default: s << *p;
          }
        }
        else
  ...
  void CommonLogger::operator()(const std::string& category, Level level,
                                boost::posix_time::ptime time, const std::string& body)
  {
