...
namespace Tools
{
  const command_line::arg_descriptor<uint16_t> pool_rpc_server::arg_rpc_bind_port = {
    "rpc-bind-port", "Starts wallet as rpc server for wallet operations, sets bind port for server",
    0, true
  };
  const command_line::arg_descriptor<std::string> pool_rpc_server::arg_rpc_bind_ip = {
    "rpc-bind-ip", "Specify ip to bind rpc server", "127.0.0.1"
  };
  const command_line::arg_descriptor<std::string> pool_rpc_server::arg_rpc_user = {
    "rpc-user", "Username to use the rpc server. If authorization is not required, leave it empty",
    ""
  };
  const command_line::arg_descriptor<std::string> pool_rpc_server::arg_rpc_password = {
    "rpc-password",
    "Password to use the rpc server. If authorization is not required, leave it empty", ""
  };
  ...
  //------------------------------------------------------------------------------------------------------------------------------
  pool_rpc_server::pool_rpc_server(System::Dispatcher& dispatcher, Logging::ILogger& log,
                                   CryptoNote::IWalletLegacy& w, CryptoNote::INode& n,
                                   CryptoNote::Currency& currency, const std::string& walletFile)
  ...
  void pool_rpc_server::processRequest(const CryptoNote::HttpRequest& request,
  ...
  //------------------------------------------------------------------------------------------------------------------------------
  bool pool_rpc_server::on_getbalance(const wallet_rpc::COMMAND_RPC_GET_BALANCE::request& req,
  ...
  //------------------------------------------------------------------------------------------------------------------------------
  bool pool_rpc_server::on_transfer(const wallet_rpc::COMMAND_RPC_TRANSFER::request& req,
                                    wallet_rpc::COMMAND_RPC_TRANSFER::response& res)
  {
    ...
    for (auto& rpc_message : req.messages)
    {
      messages.emplace_back(
          CryptoNote::TransactionMessage{ rpc_message.message, rpc_message.address });
    }
    ...
    try
    {
      ...
      CryptoNote::TransactionId tx =
          m_wallet.sendTransaction(transactionSK, transfers, actualFee, extraString, req.mixin,
                                   req.unlock_time, messages, ttl);
  ...
  //------------------------------------------------------------------------------------------------------------------------------
  bool pool_rpc_server::on_optimize(const wallet_rpc::COMMAND_RPC_OPTIMIZE::request& req,
  ...
  //------------------------------------------------------------------------------------------------------------------------------
  bool pool_rpc_server::on_store(const wallet_rpc::COMMAND_RPC_STORE::request& req,
  ...
  //------------------------------------------------------------------------------------------------------------------------------
  bool pool_rpc_server::on_get_messages(const wallet_rpc::COMMAND_RPC_GET_MESSAGES::request& req,
                                        wallet_rpc::COMMAND_RPC_GET_MESSAGES::response& res)
  {
    ...
    for (uint64_t i = req.first_tx_id;
         i < res.total_tx_count && res.tx_messages.size() < req.tx_limit; ++i)
    {
  ...
  //------------------------------------------------------------------------------------------------------------------------------
  bool pool_rpc_server::on_get_payments(const wallet_rpc::COMMAND_RPC_GET_PAYMENTS::request& req,
                                        wallet_rpc::COMMAND_RPC_GET_PAYMENTS::response& res)
  {
    ...
    std::copy(std::begin(payment_id_blob), std::end(payment_id_blob),
              reinterpret_cast<char*>(&expectedPaymentId));  // no UB, char can alias any type
  ...
  bool pool_rpc_server::on_create_integrated(
      const wallet_rpc::COMMAND_RPC_CREATE_INTEGRATED::request& req,
  ...
  bool pool_rpc_server::on_get_transfers(const wallet_rpc::COMMAND_RPC_GET_TRANSFERS::request& req,
  ...
  bool pool_rpc_server::on_get_height(const wallet_rpc::COMMAND_RPC_GET_HEIGHT::request& req,
  ...
  bool pool_rpc_server::on_get_outputs(const wallet_rpc::COMMAND_RPC_GET_OUTPUTS::request& req,
  ...
  bool pool_rpc_server::on_reset(const wallet_rpc::COMMAND_RPC_RESET::request& req,
