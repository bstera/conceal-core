...
namespace
{
  std::vector<uint64_t> split(uint64_t amount, uint64_t dustThreshold)
  {
    ...
    decompose_amount_into_digits(
        amount, dustThreshold, [&](uint64_t chunk) { amounts.push_back(chunk); },
  ...
  uint64_t calculateDepositsAmount(
      ...
      const CryptoNote::Currency &currency, const std::vector<uint32_t> heights)
  {
    int index = 0;
    return std::accumulate(
        transfers.begin(), transfers.end(), static_cast<uint64_t>(0),
        [&currency, &index, heights](uint64_t sum,
                                     const CryptoNote::TransactionOutputInformation &deposit) {
          return sum + deposit.amount +
                 currency.calculateInterest(deposit.amount, deposit.term, heights[index++]);
        });
  }

  void asyncRequestCompletion(System::Event &requestFinished) { requestFinished.set(); }

  void parseAddressString(const std::string &string, const CryptoNote::Currency &currency,
  ...
  uint64_t countNeededMoney(const std::vector<CryptoNote::WalletTransfer> &destinations,
                            uint64_t fee)
  {
      ...
      uint64_t mixIn)
  {
    auto notEnoughIt = std::find_if(
        ...
        [mixIn](
            const CryptoNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount &ofa) {
      ...
      if (order.amount > static_cast<uint64_t>(std::numeric_limits<int64_t>::max()))
      {
        throw std::system_error(
            ...
            "Order amount must not exceed " +
                std::to_string(std::numeric_limits<decltype(transfer.amount)>::max()));
      }
  ...
  CryptoNote::AccountPublicAddress parseAccountAddressString(const std::string &addressString,
...
namespace CryptoNote
{
  ...
  void WalletGreen::initialize(const std::string &path, const std::string &password)
  {
    ...
    m_logger(DEBUGGING, BRIGHT_WHITE)
        << "New container initialized, public view key " << Common::podToHex(viewPublicKey);
  }
  ...
  void WalletGreen::withdrawDeposit(DepositId depositId, std::string &transactionHash)
  {
    ...
    m_logger(DEBUGGING, BRIGHT_WHITE)
        << "Withdraw deposit, id " << depositId << " found transfer for " << transfer.amount
        << " with a global output index of " << transfer.globalOutputIndex;
    ...
    std::vector<uint64_t> outputAmounts =
        split(foundMoney - 10, parameters::DEFAULT_DUST_THRESHOLD);
  ...
  std::vector<MultisignatureInput> WalletGreen::prepareMultisignatureInputs(
  ...
  void WalletGreen::createDeposit(uint64_t amount, uint64_t term, std::string sourceAddress,
                                  std::string destinationAddress, std::string &transactionHash)
  {
    throwIfNotInitialized();
    throwIfTrackingMode();
    throwIfStopped();
    ...
    uint64_t foundMoney = selectTransfers(neededMoney, 0 == 0, m_currency.defaultDustThreshold(),
    ...
    auto depositIndex = transaction->addOutput(neededMoney - fee, { destAddr }, 1, term);
    ...
    decompose_amount_into_digits(
        foundMoney - neededMoney, m_currency.defaultDustThreshold(),
    ...
    try
    {
      ...
      {
        transaction->addOutput(amountToAddress.second, *amountToAddress.first);
      }
    }
    ...
    /* Now add the other components of the transaction such as the transaction secret key,
     unlocktime since this is a deposit, we don't need to add messages or added extras beyond the
     transaction publick key */
    Crypto::SecretKey transactionSK;
    transaction->getTransactionSecretKey(transactionSK);
    transaction->setUnlockTime(0);
  ...
  void WalletGreen::decryptKeyPair(const EncryptedWalletRecord &cipher, PublicKey &publicKey,
  ...
  void WalletGreen::decryptKeyPair(const EncryptedWalletRecord &cipher, PublicKey &publicKey,
                                   SecretKey &secretKey, uint64_t &creationTimestamp) const
  {
                                                    ...
                                                    const Crypto::chacha8_iv &iv)
  {
    ...
    result.iv = iv;
    chacha8(serializedKeys.data(), serializedKeys.size(), key, result.iv,
  ...
  {
    const auto *prefix =
        reinterpret_cast<const ContainerStoragePrefix *>(m_containerStorage.prefix());
  ...
  void WalletGreen::loadSpendKeys()
  {
    ...
    for (size_t i = 0; i < m_containerStorage.size(); ++i)
    {
      WalletRecord wallet;
      uint64_t creationTimestamp;
      decryptKeyPair(m_containerStorage[i], wallet.spendPublicKey, wallet.spendSecretKey,
                     creationTimestamp);
      ...
      {
        throwIfKeysMissmatch(wallet.spendSecretKey, wallet.spendPublicKey,
      ...
      wallet.container = reinterpret_cast<CryptoNote::ITransfersContainer *>(
          i);  // dirty hack. container field must be unique
                                          ...
                                          const Crypto::SecretKey &viewSecretKey)
  {
    ...
    {
      m_logger(ERROR, BRIGHT_RED) << "initializeWithViewKey(" << Common::podToHex(viewSecretKey)
                                  << ") Failed to convert secret key to public key";
      throw std::system_error(make_error_code(CryptoNote::error::KEY_GENERATION_ERROR));
    }

    initWithKeys(path, password, viewPublicKey, viewSecretKey);
    m_logger(INFO, BRIGHT_WHITE) << "Container initialized with view secret key, public view key "
                                 << Common::podToHex(viewPublicKey);
  }
  ...
  void WalletGreen::deleteOrphanTransactions(
      const std::unordered_set<Crypto::PublicKey> &deletedKeys)
  {
  ...
  void WalletGreen::saveWalletCache(ContainerStorage &storage, const Crypto::chacha8_key &key,
                                    WalletSaveLevel saveLevel, const std::string &extra)
  {
    ...
    WalletSerializerV2 s(*this, m_viewPublicKey, m_viewSecretKey, m_actualBalance, m_pendingBalance,
                         m_lockedDepositBalance, m_unlockedDepositBalance, m_walletsContainer,
                         m_synchronizer, m_unlockTransactionsJob, transactions, transfers,
                         m_deposits, m_uncommitedTransactions, const_cast<std::string &>(extra),
  ...
  void WalletGreen::initTransactionPool()
  {
    ...
    std::transform(
        m_uncommitedTransactions.begin(), m_uncommitedTransactions.end(),
                                 ...
                                 const Crypto::SecretKey &viewSecretKey)
  {
    ...
    ContainerStorage newStorage(path, Common::FileMappedVectorOpenMode::CREATE,
                                             ...
                                             ContainerStorage &dst, const chacha8_key &dstKey)
  {
    ...
    for (auto &encryptedSpendKeys : src)
    {
      ...
      // push_back() can resize container, and dstPrefix address can be changed, so it is requested
      // for each key pair
  ...
  void WalletGreen::copyContainerStoragePrefix(ContainerStorage &src, const chacha8_key &srcKey,
                                               ContainerStorage &dst, const chacha8_key &dstKey)
  {
                                 ...
                                 const std::string &extra)
  {
    ...
    try
    {
      ...
      ContainerStorage newStorage(path, FileMappedVectorOpenMode::CREATE,
  ...
  void WalletGreen::convertAndLoadWalletFile(const std::string &path,
                                             std::ifstream &&walletFileStream)
  {
    WalletSerializer s(*this, m_viewPublicKey, m_viewSecretKey, m_actualBalance, m_pendingBalance,
                       m_walletsContainer, m_synchronizer, m_unlockTransactionsJob, m_transactions,
    ...
    m_containerStorage.open(tmpPath.string(), Common::FileMappedVectorOpenMode::CREATE,
                            sizeof(ContainerStoragePrefix));
    ContainerStoragePrefix *prefix =
        reinterpret_cast<ContainerStoragePrefix *>(m_containerStorage.prefix());
    ...
    for (auto spendKeys : m_walletsContainer.get<RandomAccessIndex>())
    {
      m_containerStorage.push_back(encryptKeyPair(
          spendKeys.spendPublicKey, spendKeys.spendSecretKey, spendKeys.creationTimestamp));
      incNextIv();
    }
                                    ...
                                    std::string &extra)
  {
    ...
    WalletSerializerV2 s(*this, m_viewPublicKey, m_viewSecretKey, m_actualBalance, m_pendingBalance,
                         m_lockedDepositBalance, m_unlockedDepositBalance, m_walletsContainer,
                         m_synchronizer, m_unlockTransactionsJob, m_transactions, m_transfers,
                         m_deposits, m_uncommitedTransactions, extra, m_transactionSoftLockTime);

    Common::MemoryInputStream containerStream(contanerData.data(), contanerData.size());
    s.load(containerStream,
  ...
  void WalletGreen::loadContainerStorage(const std::string &path)
  {
    try
    {
      ...
      uint64_t creationTimestamp;
      decryptKeyPair(prefix->encryptedViewKeys, m_viewPublicKey, m_viewSecretKey,
                     creationTimestamp);
                                                ...
                                                const void *containerData, size_t containerDataSize)
  {
    ...
    chacha8(containerData, containerDataSize, key, suffixIv,
  ...
  void WalletGreen::load(const std::string &path, const std::string &password, std::string &extra)
  {
      ...
      if (m_containerStorage.suffixSize() > 0)
      {
        ...
        catch (const std::exception &e)
        {
          m_logger(ERROR, BRIGHT_RED)
              << "Failed to load cache: " << e.what() << ", reset wallet data";
          clearCaches(true, true);
          subscribeWallets();
        }
      }
    ...
    {
      m_logger(ERROR, BRIGHT_RED) << "Failed to read output keys!! Continue without output keys: "
                                  << e.what();
    }
    ...
    m_state = WalletState::INITIALIZED;
    m_logger(INFO, BRIGHT_WHITE) << "Container loaded, view public key "
                                 << Common::podToHex(m_viewPublicKey) << ", wallet count "
  ...
  void WalletGreen::subscribeWallets()
  {
    try
    {
      ...
      for (auto it = index.begin(); it != index.end(); ++it)
      {
        ...
        sub.syncStart.timestamp = std::max(static_cast<uint64_t>(wallet.creationTimestamp),
  ...
  std::vector<std::string> WalletGreen::createAddressList(
      const std::vector<Crypto::SecretKey> &spendSecretKeys, bool reset)
  {
                                     ...
                                     uint64_t creationTimestamp)
  {
    ...
    try
    {
      ...
      m_logger(DEBUGGING) << "Wallet added " << address << ", creation timestamp "
                          << creationTimestamp;
      return address;
    }
    catch (const std::exception &e)
  ...
  void WalletGreen::deleteAddress(const std::string &address)
  {
    ...
    std::vector<size_t> deletedTransactions;
    std::vector<size_t> updatedTransactions =
        deleteTransfersForAddress(address, deletedTransactions);
    deleteFromUncommitedTransactions(deletedTransactions);
  ...
  WalletGreen::TransfersRange WalletGreen::getTransactionTransfersRange(
      size_t transactionIndex) const
  {
    auto val = std::make_pair(transactionIndex, WalletTransfer());

    auto bounds =
        std::equal_range(m_transfers.begin(), m_transfers.end(), val,
  ...
  void WalletGreen::prepareTransaction(std::vector<WalletOuts> &&wallets,
                                       ...
                                       const std::vector<WalletMessage> &messages, uint64_t fee,
                                       uint64_t mixIn, const std::string &extra,
                                       uint64_t unlockTimestamp, const DonationSettings &donation,
                                       ...
                                       Crypto::SecretKey &transactionSK)
  {
    ...
    std::vector<OutputToTransfer> selectedTransfers;
    uint64_t foundMoney =
        selectTransfers(preparedTransaction.neededMoney, mixIn == 0,
                        m_currency.defaultDustThreshold(), std::move(wallets), selectedTransfers);
    ...
    uint64_t donationAmount = pushDonationTransferIfPossible(
        donation, foundMoney - preparedTransaction.neededMoney, m_currency.defaultDustThreshold(),
        preparedTransaction.destinations);
    preparedTransaction.changeAmount =
        foundMoney - preparedTransaction.neededMoney - donationAmount;

    std::vector<ReceiverAmounts> decomposedOutputs = splitDestinations(
        preparedTransaction.destinations, m_currency.defaultDustThreshold(), m_currency);
    ...
    preparedTransaction.transaction = makeTransaction(decomposedOutputs, keysInfo, messages, extra,
                                                      unlockTimestamp, transactionSK);
  }
      ...
      const TransactionParameters &transactionParameters) const
  {
    ...
    if (transactionParameters.donation.address.empty() !=
        (transactionParameters.donation.threshold == 0))
    {
      throw std::system_error(make_error_code(error::WRONG_PARAMETERS));
    }

    validateSourceAddresses(transactionParameters.sourceAddresses);
    validateChangeDestination(transactionParameters.sourceAddresses,
                              transactionParameters.changeDestination, false);
                                 ...
                                 Crypto::SecretKey &transactionSK)
  {
    ...
    PreparedTransaction preparedTransaction;
    prepareTransaction(std::move(wallets), transactionParameters.destinations,
                       transactionParameters.messages, transactionParameters.fee,
                       transactionParameters.mixIn, transactionParameters.extra,
                       transactionParameters.unlockTimestamp, transactionParameters.donation,
                       changeDestination, preparedTransaction, transactionSK);

    return validateSaveAndSendTransaction(*preparedTransaction.transaction,
                                          preparedTransaction.destinations, false, true);
  }
  ...
  size_t WalletGreen::makeTransaction(const TransactionParameters &sendingTransaction)
  {
    ...
    CryptoNote::AccountPublicAddress changeDestination = getChangeDestination(
        sendingTransaction.changeDestination, sendingTransaction.sourceAddresses);
    m_logger(DEBUGGING) << "Change address "
                        << m_currency.accountAddressAsString(changeDestination);
    ...
    prepareTransaction(std::move(wallets), sendingTransaction.destinations,
                       sendingTransaction.messages, sendingTransaction.fee,
                       sendingTransaction.mixIn, sendingTransaction.extra,
                       sendingTransaction.unlockTimestamp, sendingTransaction.donation,
                       changeDestination, preparedTransaction, txSecretKey);

    id = validateSaveAndSendTransaction(*preparedTransaction.transaction,
                                        preparedTransaction.destinations, false, false);
    return id;
  }
  ...
  size_t WalletGreen::insertOutgoingTransactionAndPushEvent(const Hash &transactionHash,
                                                            uint64_t fee, const BinaryArray &extra,
                                                       ...
                                                       WalletTransactionState state)
  {
    ...
    if (it->state != state)
    {
      m_transactions.get<RandomAccessIndex>().modify(
          it, [state](WalletTransaction &tx) { tx.state = state; });
                                                ...
                                                int64_t totalAmount)
  {
    ...
    bool r = txIdIndex.modify(it, [&info, totalAmount, &updated](WalletTransaction &transaction) {
      ...
      // If transaction was sent to daemon, it can not have CREATED and FAILED states, its state can
      // be SUCCEEDED, CANCELLED or DELETED
                                                  ...
                                                  int64_t txBalance)
  {
    ...
    {
      tx.fee = info.totalAmountIn < info.totalAmountOut ? CryptoNote::parameters::MINIMUM_FEE
                                                        : info.totalAmountIn - info.totalAmountOut;
    }
  ...
  bool WalletGreen::updateTransactionTransfers(
      size_t transactionId, const std::vector<ContainerAmounts> &containerAmountsList,
      int64_t allInputsAmount, int64_t allOutputsAmount)
  {
    ...
    {
      AccountPublicAddress address{ getWalletRecord(containerAmount.container).spendPublicKey,
                                    m_viewPublicKey };
    ...
    updated |= updateUnknownTransfers(transactionId, firstTransferIdx, myInputAddresses,
                                      knownInputsAmount, myInputsAmount, allInputsAmount, false);
    updated |= updateUnknownTransfers(transactionId, firstTransferIdx, myOutputAddresses,
                                      knownOutputsAmount, myOutputsAmount, allOutputsAmount, true);
  ...
  bool WalletGreen::updateAddressTransfers(size_t transactionId, size_t firstTransferIdx,
                                           const std::string &address, int64_t knownAmount,
                                           int64_t targetAmount)
  {
    assert((knownAmount > 0 && targetAmount > 0) || (knownAmount < 0 && targetAmount < 0) ||
    ...
    if (knownAmount != targetAmount)
    {
      ...
      else if (targetAmount == 0)
      {
        ...
        updated |=
            eraseTransfersByAddress(transactionId, firstTransferIdx, address, knownAmount > 0);
      }
      else
      {
  ...
  bool WalletGreen::updateUnknownTransfers(size_t transactionId, size_t firstTransferIdx,
                                           ...
                                           int64_t knownAmount, int64_t myAmount,
                                           int64_t totalAmount, bool isOutput)
  {
    bool updated = false;
    ...
    if (std::abs(knownAmount) > std::abs(totalAmount))
    {
      ...
      {
        updated |=
            eraseTransfersByAddress(transactionId, firstTransferIdx, std::string(), isOutput);
      }
      else
      {
        assert(std::abs(totalAmount) > std::abs(myAmount));
        updated |=
            adjustTransfer(transactionId, firstTransferIdx, std::string(), totalAmount - myAmount);
      }
    }
    ...
    else
    {
      ...
      updated |=
          adjustTransfer(transactionId, firstTransferIdx, std::string(), totalAmount - knownAmount);
    }

    return updated;
  }

  void WalletGreen::appendTransfer(size_t transactionId, size_t firstTransferIdx,
                                   const std::string &address, int64_t amount)
  {
  ...
  bool WalletGreen::adjustTransfer(size_t transactionId, size_t firstTransferIdx,
                                   const std::string &address, int64_t amount)
  {
    assert(amount != 0);
  ...
  bool WalletGreen::eraseTransfersByAddress(size_t transactionId, size_t firstTransferIdx,
                                            const std::string &address, bool eraseOutputTransfers)
  {
                                          ...
                                          bool eraseOutputTransfers)
  {
    return eraseTransfers(transactionId, firstTransferIdx,
                          [this, &knownAddresses, eraseOutputTransfers](
                              bool isOutput, const std::string &transferAddress) {
                            return eraseOutputTransfers == isOutput &&
  ...
  std::unique_ptr<CryptoNote::ITransaction> WalletGreen::makeTransaction(
      ...
      const std::vector<WalletMessage> &messages, const std::string &extra,
      uint64_t unlockTimestamp, Crypto::SecretKey &transactionSK)
  {
  ...
  size_t WalletGreen::validateSaveAndSendTransaction(
      const ITransactionReader &transaction, const std::vector<WalletTransfer> &destinations,
      bool isFusion, bool send)
  {
    ...
    Tools::ScopeExit rollbackAddingUnconfirmedTransaction([this, &transaction] {
      ...
      {
        // Ignore any exceptions. If rollback fails then the transaction is stored as unconfirmed
        // and will be deleted after wallet relaunch during transaction pool synchronization
      }
    });
  ...
  void WalletGreen::requestMixinOuts(
      const std::vector<OutputToTransfer> &selectedTransfers, uint64_t mixIn,
      std::vector<CryptoNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>
          &mixinResult)
  {
    std::vector<uint64_t> amounts;
    ...
    m_node.getRandomOutsByAmounts(std::move(amounts), mixIn, mixinResult,
                                  [&requestFinished, &mixinError, this](std::error_code ec) {
                                    mixinError = ec;
                                    this->m_dispatcher.remoteSpawn(std::bind(
                                        asyncRequestCompletion, std::ref(requestFinished)));
                                  });
                                        ...
                                        std::vector<OutputToTransfer> &selectedTransfers)
  {
    ...
    for (auto walletIt = wallets.begin(); walletIt != wallets.end(); ++walletIt)
    {
      for (auto outIt = walletIt->outs.begin(); outIt != walletIt->outs.end(); ++outIt)
      {
        ...
        {
          buckets[numberOfDigits].emplace_back(std::piecewise_construct,
        ...
        else
        {
          ...
          if (foundMoney < neededMoney)
          {
            ...
            selectedTransfers.emplace_back(
                OutputToTransfer{ std::move(out.second), std::move(out.first) });
  ...
  std::vector<CryptoNote::WalletGreen::ReceiverAmounts> WalletGreen::splitDestinations(
      const std::vector<CryptoNote::WalletTransfer> &destinations, uint64_t dustThreshold,
      const CryptoNote::Currency &currency)
  {
  ...
  CryptoNote::WalletGreen::ReceiverAmounts WalletGreen::splitAmount(
      uint64_t amount, const AccountPublicAddress &destination, uint64_t dustThreshold)
  {
  ...
  void WalletGreen::prepareInputs(
      ...
      std::vector<CryptoNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>
          &mixinResult,
      uint64_t mixIn, std::vector<InputInfo> &keysInfo)
  {
  ...
  WalletTransactionWithTransfers WalletGreen::getTransaction(
                                                                    ...
                                                                    size_t count) const
  {
    ...
    uint32_t blockIndex = static_cast<uint32_t>(
        std::distance(m_blockchain.get<BlockHeightIndex>().begin(), heightIt));
    return getTransactionsInBlocks(blockIndex, count);
  }

  std::vector<DepositsInBlockInfo> WalletGreen::getDeposits(const Crypto::Hash &blockHash,
                                                            size_t count) const
  {
    throwIfNotInitialized();
    throwIfStopped();
  ...
  std::vector<WalletTransactionWithTransfers> WalletGreen::getUnconfirmedTransactions() const
  {
    ...
    auto lowerBound =
        m_transactions.get<BlockHeightIndex>().lower_bound(WALLET_UNCONFIRMED_TRANSACTION_HEIGHT);
  ...
  void WalletGreen::start() { m_stopped = false; }
  ...
  {
    // Deprecated, ignore it. New event handler is onTransactionUpdated(const Crypto::PublicKey&,
    // const Crypto::Hash&, const std::vector<ITransfersContainer*>&)
  }

  void WalletGreen::onTransactionUpdated(const Crypto::PublicKey &,
                                         const Crypto::Hash &transactionHash,
                                         const std::vector<ITransfersContainer *> &containers)
  {
    ...
    for (auto container : containers)
    {
      ...
      // Don't move this code to the following remote spawn, because it guarantees that the
      // container has the transaction
      uint64_t outputsAmount;
      bool found = container->getTransactionInformation(transactionHash, info, &inputsAmount,
                                                        &outputsAmount);
      if (found) { }
      assert(found);
  ...
  DepositId WalletGreen::insertNewDeposit(const TransactionOutputInformation &depositOutput,
  ...
  DepositId WalletGreen::insertDeposit(const Deposit &deposit, size_t depositIndexInTransaction,
                                       const Hash &transactionHash)
  {
    ...
    m_logger(DEBUGGING, BRIGHT_GREEN)
        << "New deposit created, id " << id << ", locking "
        << m_currency.formatAmount(deposit.amount) << " ,for a term of " << deposit.term
        << " blocks, at block " << deposit.height;

    return id;
  }

  /* Process transactions, this covers both new transactions AND confirmed transactions */
  void WalletGreen::transactionUpdated(TransactionInformation transactionInfo,
                                       const std::vector<ContainerAmounts> &containerAmountsList)
  {
    ...
    {
      auto newDepositOuts = containerAmounts.container->getTransactionOutputs(
          transactionInfo.transactionHash,
          ITransfersContainer::IncludeTypeDeposit | ITransfersContainer::IncludeStateAll);
      auto spentDepositOutputs = containerAmounts.container->getTransactionInputs(
          transactionInfo.transactionHash, ITransfersContainer::IncludeTypeDeposit);
      ...
      for (size_t i = 0; i < newDepositOuts.size(); i++)
      {
        ...
        auto id = insertNewDeposit(newDepositOuts[i], transactionId, m_currency,
      ...
      for (size_t i = 0; i < spentDepositOutputs.size(); i++)
      {
        ...
        if (depositId == WALLET_INVALID_DEPOSIT_ID)
        {
          throw std::invalid_argument(
              "processSpentDeposits error: requested deposit doesn't exist");
        }
    ...
    {
      // In some cases a transaction can be included to a block but not removed from
      // m_uncommitedTransactions. Fix it
      m_uncommitedTransactions.erase(transactionId);
    }
    ...
    for (auto containerAmounts : containerAmountsList)
    {
      ...
      if (transactionInfo.blockHeight != CryptoNote::WALLET_UNCONFIRMED_TRANSACTION_HEIGHT)
      {
        ...
        insertUnlockTransactionJob(transactionInfo.transactionHash, unlockHeight,
                                   containerAmounts.container);
      }
    }

    updated |= updateTransactionTransfers(transactionId, containerAmountsList,
  ...
  void WalletGreen::onTransactionDeleted(ITransfersSubscription *object,
                                         const Hash &transactionHash)
  {
  ...
  void WalletGreen::updateBalance(CryptoNote::ITransfersContainer *container)
  {
    ...
    container->getOutputs(transfers2, ITransfersContainer::IncludeTypeDeposit |
    ...
    container->getOutputs(transfers, ITransfersContainer::IncludeTypeDeposit |
    ...
    if (updated)
    {
      m_walletsContainer.get<TransfersContainerIndex>().modify(
          it, [actual, pending, locked, unlocked](WalletRecord &wallet) {
      ...
      m_logger(DEBUGGING, BRIGHT_WHITE)
          ...
          << ", pending " << m_currency.formatAmount(m_pendingBalance) << ", locked deposits "
                                              ...
                                              const std::string &destinationAddress)
  {
    ...
    do
    {
      ...
      std::vector<WalletMessage> messages;
      fusionTransaction = makeTransaction(std::vector<ReceiverAmounts>{ decomposedOutputs },
                                          keysInfo, messages, "", 0, txkey);
  ...
  WalletGreen::ReceiverAmounts WalletGreen::decomposeFusionOutputs(
      const AccountPublicAddress &address, uint64_t inputsAmount)
  {
  ...
  bool WalletGreen::isFusionTransaction(const WalletTransaction &walletTx) const
  {
    ...
    else
    {
      return m_currency.isFusionTransaction(
          inputsAmounts, outputsAmounts,
          0);  // size = 0 here because can't get real size of tx in wallet.
    }
  }
                                              ...
                                              bool isFusion) const
  {
    ...
    if (changeDestination.empty())
    {
      ...
      {
        message =
            std::string(isFusion ? "Destination" : "Change destination") + " address is necessary";
        m_logger(ERROR, BRIGHT_RED)
            << message << ". Source addresses size=" << sourceAddresses.size()
  ...
  IFusionManager::EstimateResult WalletGreen::estimate(
      uint64_t threshold, const std::vector<std::string> &sourceAddresses) const
  {
    System::EventLock lk(m_readyEvent);
    ...
    auto walletOuts =
        sourceAddresses.empty() ? pickWalletsWithMoney() : pickWallets(sourceAddresses);
  ...
  std::vector<WalletGreen::OutputToTransfer> WalletGreen::pickRandomFusionInputs(
      const std::vector<std::string> &addresses, uint64_t threshold, size_t minInputCount,
      size_t maxInputCount)
  {
    ...
    std::shuffle(
        bucketNumbers.begin(), bucketNumbers.end(),
        std::default_random_engine{ Crypto::rand<std::default_random_engine::result_type>() });
    ...
    uint64_t upperBound =
        selectedBucket == std::numeric_limits<uint64_t>::digits10 ? UINT64_MAX : lowerBound * 10;
  ...
  std::vector<DepositsInBlockInfo> WalletGreen::getDepositsInBlocks(uint32_t blockIndex,
                                                                    size_t count) const
  {
    if (count == 0)
    {
  ...
  std::vector<TransactionsInBlockInfo> WalletGreen::getTransactionsInBlocks(uint32_t blockIndex,
                                                                            size_t count) const
  {
    if (count == 0)
    {
  ...
  std::vector<size_t> WalletGreen::deleteTransfersForAddress(
      const std::string &address, std::vector<size_t> &deletedTransactions)
  {
    ...
    for (size_t i = 0; i < m_transfers.size(); ++i)
    {
      ...
      if ((i == m_transfers.size() - 1) || (transactionId != m_transfers[i + 1].first))
      {
        ...
        if (deletedInputs != 0)
        {
          adjustTransfer(transactionId, firstTransactionTransfer, "",
        ...
        randomIndex.modify(
            ...
            [transfersLeft, deletedInputs, deletedOutputs](WalletTransaction &transaction) {
