...
namespace Tools
{
  ...
  //------------------------------------------------------------------------------------------------------------------------------
  wallet_rpc_server::wallet_rpc_server(System::Dispatcher& dispatcher, Logging::ILogger& log,
                                          ...
                                          wallet_rpc::COMMAND_RPC_GET_TX_PROOF::response& res)
  {
    ...
    if (!req.tx_key.empty())
    {
      ...
      size_t size;
      if (!Common::fromHex(req.tx_key, &tx_key_hash, sizeof(tx_key_hash), size) ||
      ...
      if (r)
      {
        if (tx_key != tx_key2)
        {
          throw JsonRpc::JsonRpcError(
              ...
              std::string("Tx secret key was found for the given txid, but you've also provided "
    ...
    else
    {
      if (!r)
      {
        throw JsonRpc::JsonRpcError(
            WALLET_RPC_ERROR_CODE_UNKNOWN_ERROR,
            std::string("Tx secret key wasn't found in the wallet file. Provide it as the optional "
      ...
      wallet_rpc::COMMAND_RPC_GET_BALANCE_PROOF::response& res)
  {
    ...
    {
      res.signature =
          m_wallet.getReserveProof(req.amount != 0 ? req.amount : m_wallet.actualBalance(),
  ...
  bool wallet_rpc_server::on_optimize(const wallet_rpc::COMMAND_RPC_OPTIMIZE::request& req,
      ...
      wallet_rpc::COMMAND_RPC_ESTIMATE_FUSION::response& res)
  {
    if (req.threshold <= m_currency.defaultDustThreshold())
    {
      throw JsonRpc::JsonRpcError(
          WALLET_RPC_ERROR_CODE_UNKNOWN_ERROR,
          std::string("Fusion transaction threshold is too small. Threshold: " +
  ...
  //------------------------------------------------------------------------------------------------------------------------------
  bool wallet_rpc_server::on_send_fusion(const wallet_rpc::COMMAND_RPC_SEND_FUSION::request& req,
                                         wallet_rpc::COMMAND_RPC_SEND_FUSION::response& res)
  {
    ...
    size_t estimatedFusionInputsCount = m_currency.getApproximateMaximumInputCount(
        m_currency.fusionTxMaxSize(), MAX_FUSION_OUTPUT_COUNT, req.mixin);
    ...
    try
    {
      std::list<TransactionOutputInformation> fusionInputs = m_wallet.selectFusionTransfersToSend(
          req.threshold, m_currency.fusionTxMinInputCount(), estimatedFusionInputsCount);
      if (fusionInputs.size() < m_currency.fusionTxMinInputCount())
      {
        // nothing to optimize
        throw JsonRpc::JsonRpcError(
            WALLET_RPC_ERROR_CODE_UNKNOWN_ERROR,
            std::string("Fusion transaction not created: nothing to optimize for threshold " +
      ...
      CryptoNote::TransactionId tx =
          m_wallet.sendFusionTransaction(fusionInputs, 50, extraString, 0, req.unlock_time);
      if (tx == WALLET_LEGACY_INVALID_TRANSACTION_ID)
  ...
  //------------------------------------------------------------------------------------------------------------------------------
  bool wallet_rpc_server::on_get_messages(const wallet_rpc::COMMAND_RPC_GET_MESSAGES::request& req,
  ...
  bool wallet_rpc_server::on_create_integrated(
  ...
  bool wallet_rpc_server::on_get_outputs(const wallet_rpc::COMMAND_RPC_GET_OUTPUTS::request& req,
