...
namespace Common
{
  ...
  class FileMappedVector : public EnableIfPod<T>::type
  {
   ...
   private:
    template<class F>
    void atomicUpdate(uint64_t newSize, uint64_t newCapacity, uint64_t newPrefixSize,
                      uint64_t newSuffixSize, F&& func);
    template<class F>
    void atomicUpdate0(uint64_t newCapacity, uint64_t newPrefixSize, uint64_t newSuffixSize,
                       F&& func);

    void open(const std::string& path, uint64_t prefixSize);
    void create(const std::string& path, uint64_t initialCapacity, uint64_t prefixSize,
                uint64_t suffixSize);
  ...
  template<class T>
  FileMappedVector<T>::FileMappedVector(const std::string& path, FileMappedVectorOpenMode mode,
                                        uint64_t prefixSize)
      : m_autoFlush(true)
  {
  ...
  template<class T>
  void FileMappedVector<T>::open(const std::string& path, FileMappedVectorOpenMode mode,
                                 uint64_t prefixSize)
  {
    assert(!isOpened());
                                                                    ...
                                                                    const_iterator last)
  {
    ...
    atomicUpdate(newSize, capacity(), prefixSize(), suffixSize(),
                                                                     ...
                                                                     InputIterator last)
  {
    ...
    atomicUpdate(newSize, newCapacity, prefixSize(), suffixSize(),
                 [this, position, first, last](value_type* target) {
                   ...
                   std::copy(position, cend(),
                             target + position.index() + std::distance(first, last));
                 });
  ...
  template<class T>
  template<class F>
  void FileMappedVector<T>::atomicUpdate(uint64_t newSize, uint64_t newCapacity,
                                         uint64_t newPrefixSize, uint64_t newSuffixSize, F&& func)
  {
  ...
  template<class T>
  template<class F>
  void FileMappedVector<T>::atomicUpdate0(uint64_t newCapacity, uint64_t newPrefixSize,
                                          uint64_t newSuffixSize, F&& func)
  {
  ...
  template<class T>
  void FileMappedVector<T>::create(const std::string& path, uint64_t initialCapacity,
                                   uint64_t prefixSize, uint64_t suffixSize)
  {
    m_file.create(path, prefixSize + metadataSize + initialCapacity * valueSize + suffixSize,
                  false);
    m_path = path;
    m_prefixSize = prefixSize;
