...
namespace Common
{
  ...
  class StringView
  {
   public:
    ...
    // The behavior is undefined unless 'stringData' != 'nullptr' || 'stringSize' == 0. Input state
    // can be malformed using poiner conversions.
    ...
    // The behavior is undefined unless 'other' 'StringView' is in defined state, that is 'data' !=
    // 'nullptr' || 'size' == 0
    ...
    // The behavior is undefined unless 'StringView' was initialized and 'startIndex' <= 'endIndex'
    // and 'endIndex' <= 'size'.
    ...
    // The behavior is undefined unless 'StringView' was initialized and 'startIndex' <= 'size' and
    // 'startIndex' + 'sliceSize' <= 'size'.
