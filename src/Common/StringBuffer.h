...
namespace Common
{
  ...
  class StringBuffer
  {
   public:
    ...
    // Copies string from 'stringData' to 'StringBuffer'.
    // The behavior is undefined unless ('stringData' != 'nullptr' || 'stringSize' == 0) &&
    // 'stringSize' <= 'MAXIMUM_SIZE'. Input state can be malformed using poiner conversions.
    ...
    // The behavior is undefined unless 'startIndex' <= 'size' and 'sliceSize' <= 'size' -
    // 'startIndex'.
    ...
    // The behavior is undefined unless 'startIndex' <= 'size' and 'cutSize' <= 'size' -
    // 'startIndex'.
    ...
    // The behavior is undefined unless 'index' <= 'size' and 'stringView.size()' <= 'MAXIMUM_SIZE'
    // - 'size'.
    ...
    // Overwrites 'StringBuffer' starting at 'index' with 'stringView', possibly expanding
    // 'StringBuffer'. The behavior is undefined unless 'index' <= 'size' and 'stringView.size()' <=
    // 'MAXIMUM_SIZE' - 'index'.
