    ...
    if (!stream.good())
    {
      if (stream.eof())
      {
        throw std::system_error(
            make_error_code(CryptoNote::error::HttpParserErrorCodes::END_OF_STREAM));
      }
      else
      {
        throw std::system_error(
            make_error_code(CryptoNote::error::HttpParserErrorCodes::STREAM_NOT_GOOD));
      }
    }
  }
  ...
  bool HttpParser::readHeader(std::istream& stream, std::string& name, std::string& value)
  {
    ...
    while (stream.good() && c != '\r')
    {
      if (c == ':')
      {
        ...
        {
          throw std::system_error(
              make_error_code(CryptoNote::error::HttpParserErrorCodes::EMPTY_HEADER));
        }
